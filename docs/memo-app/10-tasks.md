...
* [ ] A11Y-02 accessibility属性の付与（role / label / hint / state。`29` §8）。部分適用済み：プロンプト集＝バッチ5。**最終統合バッチ（2026-07-15）で保存/キャンセル/編集/削除/アーカイブ/書き出し/コピー/整理/GitHubアップロード/トークン削除/再試行へ role・label・state（disabled/busy）、結果ボタンへ `accessibilityLiveRegion`、見出しへ header を付与（`30` §8.7）**。TalkBack実機読み上げは未確認のため全体は未完了のまま
...
* [ ] A11Y-04 文字拡大の確認（`30` §11）。部分：**最終統合バッチはレイアウトを変更しておらず新たな崩れを持ち込んでいないことをコードレビューで確認（`30` §8.7）**。実ブラウザ/実機での文字拡大確認は未実施のため全体は未完了のまま
