...
* [ ] HOME-01 MindHub入口への再構成（`28` §7.1〜7.3）。依存：UI-01・UI-02・IA-01・IA-02、versionCode 4基準確認（依存は2026-07-14時点ですべて解消済み。バッチ2で着手）
* [ ] HOME-02 主要機能カードの優先順位表現（`28` §6.2。カード名・順序は`28` §7.2で確定済み）。依存：HOME-01
* [ ] HOME-03 最近の軽量メモ（memosのみ）初期最大3件＋ホーム内展開（「すべて表示」／「3件に戻す」等。別ルート追加なし。`28` §7.2）。依存：HOME-01・IA-01
* [ ] HOME-04 設定の補助位置への移動（補助導線とすることは確定済み。AppHeader右かホーム内補助位置かは既存実装調査後に共有AppHeaderを不必要に変更しない最小構成を選ぶ＝`11` §16）。依存：HOME-01
* [ ] HOME-05 スマホ幅確認（360〜1024の6幅。`30` §5）。依存：HOME-01〜04
