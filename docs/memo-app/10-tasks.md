...
* [ ] 16A-3 ホーム領域分離（`31` §7.3）：主要4カード固定／最近のさくっとメモ一覧だけスクロール／3件・全件切り替え／FAB・空・エラー
  * 仕様書：`31`
  * 変更対象候補：`app/index.tsx`
  * 完了条件：`31` Gate C（ホームWeb確認）
  * Web確認：6幅でカード固定・一覧のみスクロール・空/読み込み/エラー/3件/全件
  * Android実機確認：Safe Area含む実機でカード固定・一覧スクロール
  * 回帰確認：ホームから4機能へ遷移・Console/pageerror/unhandledrejection 0件
