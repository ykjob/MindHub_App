...
### 15.1 バックアップ・復元・外部取り込み（将来候補。正本 `34-backup-restore-and-external-import.md`）

将来候補・実装時期未定・現在のPhase対象外。正本は `34`。着手が決まった後に子タスクへ分割する。いずれも自動同期ではない。完了チェックは付けない。

* 完全JSONバックアップ・復元（memos・notesの完全JSONバックアップ／新規環境へのJSON復元＝元ID維持／既存環境へのJSON統合と競合確認）
* GitHubからのさくっとメモMarkdown取り込み（手動・個別。旧形式互換）
* 共有コピー（後順位。他者メモの取り込み＝新ID発行）

### 15.2 さくっとメモのブレインダンプ整理・外部AI受け渡し拡張構想（**未承認構想**。構想正本 `35-quick-memo-brain-dump-and-ai-handoff.md`）

**未承認・未実装・実装時期未定・現在のPhase対象外。** Phase 16Cのタスク・完了条件には含めない。Phase 16C完了後に、既存実装と照合して実装単位へ分解して検討する（新しいPhase番号は作らない）。完了チェックは付けない。

* 音声ブレインダンプ／整理待ち状態・本人整理欄／用途別プロンプトのアプリ内編集（標準値・編集値・標準復元・`{{本文}}`変数）／カスタムボタン構想／課金境界の仮説／AI結果保存構想
* 共有・コピー・OS共有・守秘は `33`（Phase 16C）を再利用（二重実装しない）。プロンプト編集DBは `12`（Phase 8〜9）との関係を実装前に整理する。主な未決定は `11-open-issues.md` §19、詳細は `35` §15
...
### 18.1 名称整理（2026-07-27実装／2026-07-28 Codex最終再レビュー合格／PR #1 merge済み・main反映済み）

利用者向け名称を MindHub／さくっとメモへ統一し、FlowDock を旧名称・内部由来・旧形式互換・歴史的記録に限定する。**名称境界の詳細な正本は `01` §1.10**（互換識別子一覧・名称方針の詳細は §1.10 を参照。ここでは重複転載しない）。状態：**実装完了・Codex最終レビュー合格（Critical/High/Medium/Low すべて0件）。合格対象コミット `1f368673337834336b98f4285a054b8da7a6de45`。**PR #1 merge済み・main反映済み**。EAS APKビルドは未実施で、Androidランチャー表示名はPhase 16全体を含む次回の更新APKで最終確認する**。経緯：初回実装→Codex初回レビュー「要修正・再レビュー」→文書整合と設定画面補足を修正→Codex2回目レビューのMedium指摘（`30` §14 UX-04表の「app.json変更は時期未確定」）を修正→Codex最終再レビュー合格。
...
## 21. Phase 16：さくっとメモ・質問タイミング・外部共有（2026-07-23 追加）

仕様正本：`31`（16A）・`32`（16B）・`33`（16C）。**状態（2026-08-08時点）**：16A＝実装済み（Expo Go確認済み／EAS APK未確認）、16B＝実装済み（Web・Expo Go確認済み／EAS APK未確認）、16C＝**完了（2026-08-08）**。実装済み・Web確認済み・CodexレビューPASS・PR #3でmain反映済み（merge commit `b96cc66`。レビュー経緯は `33` §20〜§23）。Pixel＋Expo Go事前確認PASS（2026-08-05。`33` §23.6）。**EAS preview APK versionCode 10（Build ID `10bedf3b-31c3-4fa7-aefe-73f62f960b5e`／対象commit `8706d7d`）をPixelへ上書きインストールし、最終受入K1〜K22すべてPASS（2026-08-08。`33` §23.7）。`33` Gate A〜G PASS・`32` Gate D PASS。Phase 16C最終完了**。既存ルートの名称変更・削除・移動・構成破壊はしない（Phase 16Cでは `33` §13 を満たす共通共有確認ルートの**追加のみ可**）。DB・依存関係・守秘既定は変更しない。各タスクは実装後に `30` の幅別・A11Y基準を参照して検証し、判定を記録する（推測で合格にしない）。

実装順・依存の要点（詳細は `09` Phase 16「依存関係・実装順」）。

* 16Aは16B・16Cに依存せず独立して着手できる
* 16B（16B-1判定ロジック→16B-2判定画面/導線→16B-3完成質問文の任意保存）は、16Cの共有基盤がなくても実装・確認できる
* 16C（16C-1共通共有基盤→16C-2プロンプト集/さくっとメモ→16C-3現場適応）
* 完成**質問文**の保存は16B-3、**報告文**の保存と質問・報告の共有接続は16C-3。現場適応専用保存処理を共用し二重実装しない
* `32` Gate D（共有・守秘）は16C-1／16C-3統合時の連携Gateで、16Bの判定・保存の完了を16C未実装だけで不合格にしない
...
* [x] 16A-2 キーボード・フッター（`31` §7.2・§16）＝**完了（2026-07-24・Expo Go確認済み／EAS APK未確認）**：Android/iOS/Webの保存・キャンセル操作／二重操作・失敗状態／入力欄との非重複
  ...
  * 完了条件：`31` Gate B・Gate D
  * Web確認：ブラウザ幅別に保存・キャンセルが画面外へ出ない
  * Android実機確認：キーボード表示中に保存・キャンセルを押せる／最大フォントの結果を記録
  * 回帰確認：保存成功時遷移・キャンセル遷移の現行動作維持・QMEMO-08の保存状態が退行していない
* [ ] 16A-3 ホーム領域分離（`31` §7.3）＝**実装済み・Web確認の一部未実施・確認待ち（2026-07-24）**：主要4カード固定／最近のさくっとメモ一覧だけスクロール／3件・全件切り替え／FAB・空・エラー
  * 仕様書：`31`
  * 変更対象：`app/index.tsx`（実施済み。カード＋見出しを固定 `View` へ分離、`FlatList` を `flex:1`＋`contentContainer` `flexGrow:1` で残り領域スクロール。取得・保存・route・DB・依存・app.json・versionCode 無変更）
  * 完了条件：`31` Gate C（ホームWeb確認）。**現状＝一部確認済み**（下記）。読み込み中・取得失敗・再試行のライブ再現とFAB非重複の精密確認が残るため**完了扱いにしない**
  * Web確認（**実施済み・合格**）：6幅（360/390/412/480/768/1024）横スクロールなし・4カード表示／空状態でカード＋見出し＋空メッセージ共存・カード順維持／初期3件→`すべて表示`5件→`3件に戻す`3件／スクロールでカード固定（top 92不変）・見出し固定（role=heading top 295不変）・一覧のみスクロール／console.error・pageerror・unhandledrejection・VirtualizedList入れ子警告 0件（ヘッドレスChrome・専用ポート8114・scratchpad隔離）
  * Web確認（**未実施＝コードレビューのみ**）：読み込み中・取得失敗・再試行のライブ再現（loading/error経路は基準版と同一・本変更で未改変）／FABと最終項目・切替ボタンの重なり精密測定（`paddingBottom:96` 維持）
  * Android実機確認：**Expo Go 未確認・EAS APK 未確認**（Safe Area含む実機でのカード固定・一覧スクロールは 16A-1・16A-2 合流後に Phase 16A 全体で確認）
  * 回帰確認：ホームから4機能への遷移・4カード名称/順序/遷移・設定ボタン・FAB・メモ読み込み・空状態は Web で確認済み（Console等0件）。機械確認：`npx tsc --noEmit` 合格／`npx expo export --platform web` 成功／`npx expo-doctor` 17/18（失敗1件は既存の `expo` パッチ版差＝本変更と無関係）／`git diff --check` 問題なし
...
* [x] 16C-1 共通共有基盤（`33` §8・§10・§13・§19）：共有確認画面／編集・コピー・OS共有／Web fallback／二重操作・エラー状態 — **完了（2026-08-08）**：実装済み・Web確認済み・CodexレビューPASS・main反映済み／Expo Go事前確認PASS（2026-08-05）／**EAS APK versionCode 10 のPixel最終受入PASS（2026-08-08。K1〜K22）**・`33` Gate A〜G PASS・`32` Gate D PASS
  * 仕様書：`33`（実装記録は `33` §20）
  * 実装：`app/share/confirm.tsx`（route `/share/confirm`）・`src/features/share/shareHandoff.ts`・`src/utils/share.ts`・`app/_layout.tsx`
  * 完了条件：`33` Gate E（Web）＝**確認済み**・Gate G（表示文言）＝**確認済み**
  * Web確認：共有対応/非対応ブラウザ（`navigator.share` 差し替えで成功・キャンセル・失敗・非対応の4分岐）・6幅・直アクセスで空状態＋戻る＝**合格**
  * Android実機確認：**Pixel＋Expo Go事前確認＝合格（2026-08-05）**／**versionCode 10 APK最終受入＝合格（2026-08-08。Metroなし単体起動・既存データ保持・OS共有起動・キャンセル後復帰・日本語/改行/長文保持・多重起動なし・クラッシュなし・誤った完了表示なし。K1〜K16）**。`33` **Gate D PASS**。ChatGPT未インストール端末の実機テストは実施していないが、**特定アプリを直接起動せずAndroid標準OS共有を利用する実装**であることを確認し、Gate基準を実装に合わせて整理した（`33` §17.4・§23.7）
  * 回帰確認：既存コピー機能の維持＝**確認済み**（コピー成功経路も**versionCode 10 実機で確認済み**＝K22）
* [x] 16C-2 プロンプト集・さくっとメモ（`33` §7.1・§7.2・§11）：共有入口／5用途／プロンプト固定マッピング／通常注意／新規記録作成への引き継ぎ — **完了（2026-08-08）**：実装済み・Web確認済み・CodexレビューPASS・main反映済み／Expo Go事前確認PASS（2026-08-05）／**EAS APK versionCode 10 のPixel最終受入PASS（2026-08-08。K1〜K22）**・`33` Gate A〜G PASS・`32` Gate D PASS
  * 仕様書：`33`（用途マッピングと用途3の例外は `33` §20.2〜§20.4）・`06`・`14`
  * 実装：`app/prompts/index.tsx`・`app/memo/[id]/index.tsx`・`src/features/share/shareTargets.ts`・`app/notes/create.tsx`
  * 用途3「質問内容を整理する」は既存42件に意味の合う定義がないため、共有専用の固定依頼文を使う（プロンプト集・DBテンプレート・`35` の編集対象に含めない。`33` §20.3）
  * 完了条件：`33` Gate A（プロンプト集）＝**確認済み**・Gate B（さくっとメモ）＝**確認済み**・Gate F（保存）＝**Web確認済み**
  * Web確認：5用途・既定「内容を整理する」・プロンプト＋本文の編集・元メモ非変更・42件/検索/絞り込み非破壊＝**合格**（単体32/32＋Web）
  * Android実機確認：**Pixel＋Expo Go事前確認＝合格（2026-08-05）**／**versionCode 10 APK最終受入＝合格（2026-08-08。プロンプト集・さくっとメモから共有画面起動、ChatGPT選択、自動送信なし、キャンセル後復帰、日本語/改行/長文・編集文章の維持、多重起動なし、コピー成功。K6〜K16・K22）**
  * 回帰確認：本文コピー・編集・削除・GitHub連携＝本文コピーと画面構成は確認済み（GitHub実通信は対象外・従来どおり未確認）
* [x] 16C-3 現場適応の共有接続（`33` §7.3・§7.4・§8.4・§19）：質問・報告の共有接続／必須守秘チェック／**報告文**の任意保存 — **完了（2026-08-08）**：実装済み・Web確認済み・CodexレビューPASS・main反映済み／Expo Go事前確認PASS（2026-08-05）／**EAS APK versionCode 10 のPixel最終受入PASS（2026-08-08。K1〜K22）**・`33` Gate A〜G PASS・`32` Gate D PASS
  * 仕様書：`33`（守秘チェックの適用範囲は `33` §20.5）・`21`〜`23`・`32`
  * 依存：16C-1（共通共有基盤）。完成質問文の保存そのものは16B-3（本タスクは共有確認画面への接続を担当）
  * 実装：`app/workplace/question.tsx`・`app/workplace/report.tsx`（共有導線）・`src/features/workplace/workplaceService.ts`（`saveReportNote`。16B-3の `saveWorkplaceNote` を共用し二重実装なし）
  * 守秘3チェックは**共有確認画面内のコピー・共有にのみ必須**。`/workplace/question`・`/workplace/report` の既存コピーは変更していない（`33` §20.5）
  * 完了条件：`33` Gate C（現場適応）・Gate F（保存）・Gate G（表示文言）＝**PASS**。`32` Gate D（共有・守秘）＝**PASS**（versionCode 10 APKでのAndroid確認により通過。`32` §18.4）
  * Web確認：3守秘チェック完了前はコピー・共有不可／**守秘確認後に本文を編集した場合はチェックを解除し再確認を必須にする（`33` §8.4.1）**／保存時 private・Git候補false 強制（`/notes` 詳細で確認）／会社固有情報の入力を促す文言がない＝**合格**
  * Android実機確認：**Pixel＋Expo Go事前確認＝合格（2026-08-05）**／**versionCode 10 APK最終受入＝合格（2026-08-08。質問・報告の共有、守秘3チェック制御、本文変更後の全解除と再確認、キャンセル後の文章維持、コピー成功、誤った完了表示なし。K8・K10・K16・K17〜K22）**。`32` Gate D・`33` Gate D とも **PASS**（Gate EはWeb確認済み）
  * 回帰確認：現場適応5場面・終業前メモ保存導線＝質問・報告の既存コピー／保存／完了導線は維持を確認。終業前メモ保存導線は今回未再確認
