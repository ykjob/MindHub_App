...
* [ ] STATE-01 一覧系画面へのListStateView適用（`30` §9）。依存：UI-02・NOTES-03（部分適用済み：ホーム＝バッチ2・メモ管理一覧＝バッチ3・現場適応入口の再開メモ表示＝バッチ4・**さくっとメモ詳細/編集の読み込み状態＝バッチ6B-2（2026-07-15。loading/load-error/not-foundを区別。`30` §8.6.2）**。プロンプト集の検索結果・記録確認詳細等が残るため全体は未完了のまま）
* [ ] STATE-02 エラー表示（再試行導線・データ非消失の明示）。依存：STATE-01（部分適用済み：ホーム・メモ管理一覧・現場適応入口はListStateViewのerror＋再試行を接続済み。**さくっとメモ詳細/編集はバッチ6B-2でerror＋再試行を接続し、テスト時のgetMemoById例外注入で実エラー表示・再試行復旧を確認済み（`30` §8.6.2）**。他画面の実エラー表示は未再現のため全体は未完了のまま）
* [ ] STATE-03 操作結果表示（保存・コピーの成功／失敗を文字で）。依存：UI-02（部分適用済み：プロンプト集のコピー結果をStatusMessageへ共通化＝バッチ5（2026-07-14）。**さくっとメモ作成・編集の保存失敗を `showMessage` で文字通知＝バッチ6B-2（2026-07-15。従来の無反応を解消。入力本文・カテゴリは維持。`30` §8.6.2）**。DETAIL-01の詳細画面本文コピー・コピー結果のStatusMessage統一・記録確認/現場適応の保存結果等が残るため全体は未完了のまま）
