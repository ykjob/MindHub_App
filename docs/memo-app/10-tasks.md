...
仕様正本：`31`（16A）・`32`（16B）・`33`（16C）。**状態（2026-08-02時点）**：16A＝実装済み（Expo Go確認済み／EAS APK未確認）、16B＝実装済み（Web・Expo Go確認済み／EAS APK未確認）、16C＝**実装済み・Web確認済み（Codex初回レビューのMedium 4・Low 4、再レビューのHigh 1・Low 1をいずれも修正済み・再々レビュー待ち・EAS APK更新待ち・更新APKでのPixel確認待ち。対応内容は `33` §21・§22）**。既存ルートの名称変更・削除・移動・構成破壊はしない（Phase 16Cでは `33` §13 を満たす共通共有確認ルートの**追加のみ可**）。DB・依存関係・守秘既定は変更しない。各タスクは実装後に `30` の幅別・A11Y基準を参照して検証し、判定を記録する（推測で合格にしない）。
...
* [ ] 16C-1 共通共有基盤（`33` §8・§10・§13・§19）：共有確認画面／編集・コピー・OS共有／Web fallback／二重操作・エラー状態 — **実装済み・Web確認済み（2026-08-02）／Codex初回レビュー（Medium 4・Low 4）・再レビュー（High 1・Low 1）の指摘を修正済み・再々レビュー待ち・EAS APK更新待ち・更新APKでのPixel確認待ち**
  * 仕様書：`33`（実装記録は `33` §20）
...
* [ ] 16C-2 プロンプト集・さくっとメモ（`33` §7.1・§7.2・§11）：共有入口／5用途／プロンプト固定マッピング／通常注意／新規記録作成への引き継ぎ — **実装済み・Web確認済み（2026-08-02）／Codex初回レビュー（Medium 4・Low 4）・再レビュー（High 1・Low 1）の指摘を修正済み・再々レビュー待ち・EAS APK更新待ち・更新APKでのPixel確認待ち**
...
* [ ] 16C-3 現場適応の共有接続（`33` §7.3・§7.4・§8.4・§19）：質問・報告の共有接続／必須守秘チェック／**報告文**の任意保存 — **実装済み・Web確認済み（2026-08-02）／Codex初回レビュー（Medium 4・Low 4）・再レビュー（High 1・Low 1）の指摘を修正済み・再々レビュー待ち・EAS APK更新待ち・更新APKでのPixel確認待ち**
