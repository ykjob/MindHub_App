...
* [x] HOME-02 主要機能カードの優先順位表現（`28` §6.2。カード名・順序は`28` §7.2で確定済み）。依存：HOME-01（2026-07-14完了＝バッチ2。表示順（上段＝最重要：すぐメモする・仕事を整理する）＋上段2カードの左アクセント（brand色3px）で表現。見た目の差の十分性はGate 7再評価で最終判定）
