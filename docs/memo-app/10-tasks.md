...
* [ ] IA-01 ホームの役割確定：正本は `28` §7で定義済み。残る判断＝最近のメモの取得元（memos / notes / 両方。`11` §16）
* [ ] IA-02 機能名称の画面表示確定：クイックメモ表記・現場適応モードの短い表示名（`11` §16）。app.json変更は別判断
...
* [ ] HOME-01 MindHub入口への再構成（`28` §7.1〜7.3）。依存：UI-01・UI-02・IA-01・IA-02、versionCode 4基準確認
* [ ] HOME-02 主要機能カードの優先順位表現（`28` §6.2）。依存：HOME-01
* [ ] HOME-03 最近のメモ最大3件＋すべて見る導線。依存：HOME-01・IA-01
* [ ] HOME-04 設定の補助位置への移動。依存：HOME-01
