...
## 21. Phase 16：さくっとメモ・質問タイミング・外部共有（2026-07-23 追加。未着手）

仕様正本：`31`（16A）・`32`（16B）・`33`（16C）。**すべて未着手**（文書のみ承認済み・実装前）。既存ルートの名称変更・削除・移動・構成破壊はしない（Phase 16Cでは `33` §13 を満たす共通共有確認ルートの**追加のみ可**）。DB・依存関係・守秘既定は変更しない。各タスクは実装後に `30` の幅別・A11Y基準を参照して検証し、判定を記録する（推測で合格にしない）。
...
* [ ] 16C-1 共通共有基盤（`33` §8・§10・§13・§19）：共有確認画面／編集・コピー・OS共有／Web fallback／二重操作・エラー状態
  * 仕様書：`33`
  * 変更対象候補：共通共有確認画面（route名・component名・hook名は実装判断）
  * 完了条件：`33` Gate E（Web）・Gate G（表示文言）
  * Web確認：共有対応/非対応ブラウザ・6幅・Web直アクセスで漏えい/空白/クラッシュなし
  * Android実機確認：`33` Gate D（OS共有画面が開く・未インストールでもクラッシュしない）
  * 回帰確認：既存コピー機能の維持
* [ ] 16C-2 プロンプト集・さくっとメモ（`33` §7.1・§7.2・§11）：共有入口／5用途／プロンプト固定マッピング／通常注意／新規記録作成への引き継ぎ
  * 仕様書：`33`・`06`・`14`
  * 変更対象候補：`app/prompts/index.tsx`・さくっとメモ詳細（`app/memo/[id]/index.tsx`）の共有入口・共有確認画面の用途分岐
  * 完了条件：`33` Gate A（プロンプト集）・Gate B（さくっとメモ）・Gate F（保存）
  * Web確認：5用途・既定「内容を整理する」・プロンプト＋本文の編集・元メモ非変更・42件/検索/絞り込み非破壊
  * Android実機確認：共有画面が開く・長文/日本語/改行が欠けない
  * 回帰確認：本文コピー・編集・削除・GitHub連携
* [ ] 16C-3 現場適応の共有接続（`33` §7.3・§7.4・§8.4・§19）：質問・報告の共有接続／必須守秘チェック／**報告文**の任意保存
  * 仕様書：`33`・`21`〜`23`・`32`
  * 依存：16C-1（共通共有基盤）。完成質問文の保存そのものは16B-3（本タスクは共有確認画面への接続を担当）
  * 変更対象候補：現場適応の質問・報告出力からの共有導線・報告文の現場適応専用保存処理（16B-3の質問保存処理を共用し二重実装しない）
  * 完了条件：`33` Gate C（現場適応）・Gate F（保存）・Gate G（表示文言）。`32` Gate D（共有・守秘）はここで通過させる
  * Web確認：3守秘チェック完了前はコピー・共有不可／保存時 private・Git候補false 強制／会社固有情報の入力を促す文言がない
  * Android実機確認：`33` Gate D・Gate E
  * 回帰確認：現場適応5場面・終業前メモ保存導線
