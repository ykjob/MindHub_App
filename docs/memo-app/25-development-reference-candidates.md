...
追記：2026-07-12（目的2本柱化に伴い、柱2「現場理解カード」G-01〜G-20 を §10 に追加。技術リファレンスR-01〜R-31＝柱1、現場理解カードG-01〜G-20＝柱2。`24` §1・§6.2）。
...
* 検索性のため、必要に応じて `dev-ref` にサブ観点タグを併記してよい（例 `dev-ref,expo` `dev-ref,git`）。現場理解カード（G-01〜G-20）のサブタグは6種で確定（2026-07-12。§10.3 / `11` §15）。技術リファレンス（R）側のサブタグは引き続き任意
...
* **面接質問プロンプト（面接想定質問・回答練習用プロンプト）**：今回の主目的（実務理解リファレンス）ではないため対象外（2026-07-12決定）。必要になったら別途、jobsearch カテゴリ等で検討する。dev-ref には混ぜない
...
1. 登録スコープ（R側）：31件全部か、§3の高優先7件から始めるか、§6で統合した約20件か
2. 統合方針（R側）：`24` §6「1件ずつ登録」を厳守するか、§6の統合メモを許容するか
3. タグ粒度（R側）：`dev-ref` 単独か、`dev-ref` ＋ サブ観点タグ（例 `dev-ref,expo`）を併用するか（G側は6種で確定済み。§10.3）
4. Git候補の扱い（R側）：`command` 初期値 true のまま公開許容か、dev-refは当面 false 運用にするか（G側は true で確定済み。§10.4）
5. 登録の実作業（G側）：`27` の20枚を高優先5枚（§10.5）から登録するか、20枚まとめて登録するか。登録実施のタイミング

## 9. 関連仕様への参照

* 方針正本：`24-development-reference.md`（§1 2本柱、§6.2 現場理解カード）
...
## 10. 現場理解系カード（G-01〜G-20、2026-07-12追加）

柱2「現場理解カード」（`24` §1）の候補一覧。実際の開発現場でどう動くかの確認手順・判断基準を1枚ずつカード化する。登録用のコピペ本文（完成下書き）は `27-development-reference-fieldwork-cards.md` にあり、本節は一覧・関係整理・優先度の正本。

### 10.1 一覧（6グループ・20枚）

| ID | タイトル | サブタグ | 優先度 | 関連する既存カード・仕様 |
|----|---------|---------|--------|------------------------|
| G-01 | 現場初日に確認すること | onboarding | 中 | R-16（開始時に読む資料の固定化が実例） |
| G-02 | タスクを受け取った時に確認すること | onboarding | **高** | G-03 / G-06 |
| G-03 | 作業前に仕様書で見ること | onboarding | 中 | R-16、CLAUDE.md読み分け表、`11-open-issues.md` 運用 |
| G-04 | 既存コードを読む時の順番 | onboarding | 中 | G-05 |
| G-05 | 実装前に影響範囲を確認する方法 | onboarding | 中 | R-15（既存を壊さない）、R-24（共有定義の2系統） |
| G-06 | 質問する前に整理すること | ask-report | **高** | 現場適応モード「詰まり記録」「質問文作成」が入力支援の実例 |
| G-07 | 質問してよいタイミング | ask-report | 中 | G-06 |
| G-08 | 報告に含めるべきこと | ask-report | **高** | 現場適応モード「進捗報告作成」の出力体裁が実例 |
| G-09 | レビュー依頼前に確認すること | ask-report | 中 | R-01（検証2コマンドが実例） |
| G-10 | エラーが出た時の切り分け順 | troubleshoot | **高** | R-02（保存無反応の確認順が個別実例） |
| G-11 | 再現手順の書き方 | troubleshoot | 中 | G-10 / G-12 |
| G-12 | ログ・コンソールの見方 | troubleshoot | 中 | R-02（空catchはログに出ない） |
| G-13 | Git作業前後の確認 | git-db | 中 | R-13（ブランチ切替後npm install）、R-18（表記と実態の一致） |
| G-14 | ブランチ・commit・pushの注意 | git-db | 中 | R-14（commit/pushは指示後のみ、が本リポジトリでの運用形） |
| G-15 | DB変更時の注意 | git-db | 中 | R-09（schema_version方式）、R-10（値追加はマイグレ不要） |
| G-16 | テスト確認の考え方 | git-db | 中 | R-01（検証コマンド＋実操作確認の分離） |
| G-17 | 守秘情報を扱わない判断基準 | security | 中 | R-25 / R-26（公開除外基準）、`23` §3.1 |
| G-18 | 現場固有情報をメモに残す時の注意 | security | 中 | R-30（dev-ref＝汎用/現場適応＝private の線引き） |
| G-19 | 終業前に残すべき再開情報 | closing | **高** | R-17（終了時更新3ファイル）、現場適応モード終業前メモ |
| G-20 | 翌朝に前日作業へ戻る手順 | closing | 中 | R-16、現場適応モード翌朝再開導線 |

### 10.2 Rカード（技術リファレンス）との関係

* **Gカード＝汎用の確認手順・判断基準（親）、Rカード＝本プロジェクトで実際に起きた個別実例（子）**という役割分担。例：G-10（切り分けの一般手順）⇔ R-02（保存無反応という個別事例）
* 題材が重なる組は統合せず相互リンクで残す（G-13/G-14 ⇔ R-13/R-14/R-18、G-15 ⇔ R-09/R-10、G-17/G-18 ⇔ R-25/R-26/R-30）。Gは現場を問わず通用する手順、Rはこのリポジトリ固有の再現例のため、圧縮すると片方の用途が失われる
* `24` §3（旧版）に対象として明記されながらRカードが存在しなかった「質問前チェック」「報告前チェック」はG-06〜G-09で充足した

### 10.3 サブタグ（2026-07-12確定・6種）

全カード共通タグ `dev-ref` に加え、第2タグでグループを表す。既存 `/notes` のタグ絞り込みで「dev-ref全体」「グループ単位」の両方を引けるようにする。

| サブタグ | グループ | 対象 |
|---------|---------|------|
| onboarding | 着手前 | G-01〜G-05 |
| ask-report | 質問・報告 | G-06〜G-09 |
| troubleshoot | トラブル対応 | G-10〜G-12 |
| git-db | Git・DB・テスト | G-13〜G-16 |
| security | 守秘 | G-17〜G-18 |
| closing | 業務の区切り | G-19〜G-20 |

### 10.4 現場適応モードとの境界・Git候補

* G-06〜G-08・G-19・G-20は現場適応モードの5場面と題材が重なるが、**Gカード＝何を確認・記録すべきかの判断基準の正本、現場適応モードの画面＝その場で書くための入力支援ツール**として役割を分ける。5場面モデル（`21` §2）は変更しない。カード本文の実例欄から `/workplace` の該当画面を参照するに留める
* Git候補は `command` 初期値どおり **true**。Gカードは20枚すべて汎用内容のみで、守秘系のG-17/G-18も本文は「何を出さないか」の汎用判断基準であり現場固有情報を含まない（`27` 作成時に確認済み）

### 10.5 登録の進め方

1. 最初に登録する高優先5枚：**G-02（タスク受領時）・G-06（質問前整理）・G-08（報告内容）・G-10（エラー切り分け）・G-19（終業前再開情報）**。実務で最初に必要になる場面のカード
2. 残り15枚は中優先。高優先5枚の登録・運用に問題がなければ続けて登録する
3. 登録方式：`27` のブロックを `/notes` 作成画面へコピペ（カテゴリ `command`・タグ・タイトル・本文）。Web版DBはOPFSで外部スクリプトが触れないため（R-28）seedスクリプトは使わない。将来JSONインポート（`18`）実装後は一括投入へ切替可
