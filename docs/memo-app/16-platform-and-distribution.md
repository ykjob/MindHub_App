...
## 8. Phase 16Cのプラットフォーム差（2026-07-23追記。実装・確認状態の正本は `09-roadmap.md` Phase 16・`10-tasks.md` §21）

Phase 16C（`33-external-handoff-and-sharing.md`）のOS／ブラウザ共有は、プラットフォームで挙動が異なる。ビルド方式（EASクラウド・SDK 54）・配布区分（§17）は変更しない。

* Android：OSの共有画面（共有先一覧）を開く。ユーザーがChatGPT等を選ぶ。未インストールでもクラッシュしない
* Web：ブラウザ共有が利用可能なら共有画面を開く。利用できない場合はエラーで終了せず「この環境では共有画面を開けないため、コピーして利用してください」と案内し、コピーは引き続き利用可能にする
* iOS：Phase 16Cの必須完了条件には含めない。標準共有機能で明らかな回帰を持ち込まない（既存動作を壊さない）
* MindHubは共有画面を開くところまでを扱い、**実際に相手へ送信されたことは保証しない**（`33` §10）

ビルド方式・SDKバージョン・配布区分（自分用／家族用／配布用）は本追記で変更しない。

## 9. データ移行・復元仕様の分担（2026-07-27追記。将来方針）

PC⇔Android等のデータ移行・復元に関する仕様の分担を整理する。いずれも将来候補・実装時期未定で、本追記で既存のAPKビルド記録・端末運用手順は変更しない。

* `18-json-import-export.md`：notes向けの**初期**PC⇔Android手動移行仕様（JSONエクスポート／インポート）。34の下位仕様として残す
* `34-backup-restore-and-external-import.md`：`memos` を含む**完全バックアップ・復元**、および**GitHub Markdown取り込み**の上位方針の正本
* §4の「PC / Android / iPhone間の自動同期は今回の対象外」という既存方針は維持する。34のJSON復元・Markdown取り込みはユーザーが明示的に開始する手動操作であり、自動同期ではない
