...
ビルド設定の現状（2026-07-07整備済み）。

* eas.json：preview / production とも internal distribution・APK形式
* app.json：android.package = com.ykjob.flowdock（仮置き。初回ビルド前まで変更可）、versionCode = 1
* 未実施（Expoアカウントログインが必要なためユーザー操作待ち）：`npx eas-cli login`、`eas build:configure`（EASプロジェクトID付与）、`eas build --platform android --profile preview`

インストール後の確認チェックリスト。

* [ ] 開発サーバーなしで起動する
* [ ] メモ管理（notes）：作成・一覧・詳細・編集・アーカイブ
* [ ] メモ管理：検索・絞り込み・並び替え
* [ ] Markdownプレビュー表示
* [ ] FlowDock（既存メモ）：作成・編集
* [ ] アプリ再起動後もデータが残る（端末内SQLite）
* [ ] 機内モード（オフライン）でメモ作成・編集・保存ができる
* [ ] familyカテゴリ・visibility=familyが選択・保存できる
* [ ] prompts.htmlをAndroidのブラウザで開いてコピーできる（閲覧経路はPages判断まではファイル直接送付またはGitHub経由）

既知の制約（初版では許容。将来改善候補）。

* アプリ内のプロンプトコピー・本文コピーはWeb専用実装（`src/utils/clipboard.ts` がネイティブでは常にfalse）のため、APK上では動作しない想定。プロンプトコピーはprompts.html（ブラウザ側）で代替する。expo-clipboard導入は将来判断
* Markdown書き出しはブラウザダウンロード方式（Blob + アンカー）のため、APK上では動作しない想定。書き出しはPC Web版で行う
