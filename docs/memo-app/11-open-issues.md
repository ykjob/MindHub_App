...
* **プロンプト集の状況別分類**（`28` §10.1）。決定時期：PROMPT-01着手前 → **確定（2026-07-14 バッチ5実装、同日修正）**：6入口を採用＝**問題を整理する／エラー・不具合を整理する／予定・やることを整理する／作業結果をまとめる／引き継ぎを作る／開発作業を依頼・確認する**。当初実装（旧6入口＝問題を整理する／エラーを相談する／作業を始める／報告文を作る／引き継ぎを作る／仕様・実装を確認する）は入口名からの広い推測と実際のプロンプト本文が行う処理にずれがあったため、同日に本文が行う処理へ合わせて名称・ID対応を修正した（旧「エラーを相談する」→「エラー・不具合を整理する」、旧「仕様・実装を確認する」の `design`/`research` は設計メモ・調査メモへの整形のため入口から除外し既存セクション・検索へ。`claude_prompt`・`claude_work_start` を「開発作業を依頼・確認する」へ追加）。入口→ID対応は `28` §10.1の表、実装は `app/prompts/index.tsx` 内の固定IDマッピング（`PROMPT_SITUATIONS`）。既存プロンプト本文・ID・件数（42本）・名称・所属セクション・生成処理（`promptHub.ts` / `mobilePrompts.ts` / `chatgptPrompts.ts` / `noteCategories.ts` / `generate_prompt_hub.mjs`）・HTML版（`docs/mobile-view/prompts.html`）は無変更。状況別入口は**既存42件から目的に近いものを絞り込む補助フィルター**で全42件を6入口へ無理に割り当てず、登録されていないプロンプト（就活・投資、および生活・家庭共有の `family_manual`・`household_rule`）は「すべて」・既存セクション・キーワード検索から従来どおり到達可能。生活・家庭共有を一律除外はせず、目的に自然に一致する `wife_schedule`・`outing_plan`・`shopping_memo` は「予定・やることを整理する」に個別対応。複数入口重複はどちらの目的からも自然なもの（`next_one_action`／`validation_log`／`worklog_close_summary`／`worklog`）のみ許可
* **プロンプト集の後続プロンプト候補（現行42件で不足する可能性があるもの）**（`14` §1.2「今回見送り」・§1.4の候補を統合参照）。状況別入口の運用で明確になった追加候補：**Claude Code報告確認／リポジトリ状況整理／検証結果レビュー／仕様と実装の整合確認**（前3件は `14` §1.2で見送り済みの3本と同一。重複記述は増やさずここに集約）。方針：**状況別入口を実際に使用した後、既存42件では不足すると判断した場合に追加を検討する**。**Phase 15バッチ5では追加しない**（本文・件数・生成処理は無変更）。**既存42件を維持したまま、別タスクとして追加可否・追加方法（コード追加か note_templates 実装後にDBへ追加か）を判断する**（生成方式は `14` §1.6 の決定済み事項に従う）
* **ホームの設定導線の具体配置**：→ **確定（2026-07-14 バッチ2実装）**。AppHeader右側の小さな「設定」ボタンを採用。共有AppHeader本体は無変更（既存 `right` propへ `app/index.tsx` から注入する最小構成）。幅360〜1024のWeb確認でタイトル・補助文と衝突なし・`/settings` 遷移動作を確認。Android実機（2026-07-14 APK確認2＝versionCode 6）でも主要カードと異なる補助導線としての表示・遷移とも問題なし（`30` §12.1）
* **ホームカード左アクセント削除後の見え方**：→ **問題なしを確認（2026-07-14 APK確認2）**。Android実機（versionCode 6）で主要カード4枚が同じ外観・左アクセントの残留なし・左右バランスに違和感なし（`30` §12.1。HOME-02の記録は `10` §20）
* **スマホ幅でのAppHeader補助文表示**（常時表示・省略・非表示。`29` §12）。決定時期：HEADER-01実装中
* **AI色（`#7C3AED`）の使用範囲**（プロンプト集全体か、バッジ・識別要素のみか。`29` §12）。決定時期：PROMPT系タスク着手前 → **部分的に確定（2026-07-14 バッチ5）**：プロンプト集画面（`app/prompts/index.tsx`）では状況別入口・コピー結果表示ともにAI紫色を使わず**既存ブランド色（`#2563EB`）とFilterChip既定色のみ**を採用した（新色追加なし）。ただしAI色をアプリ全体でどの範囲まで使うか（プロンプト識別バッジ等への適用可否を含む全体方針）は**引き続き未確定**であり、本項目全体の完了扱いにはしない

Phase 15内では確定しない（保留のまま）もの。

* **Webの戻るボタン視認性**（2026-07-14 versionCode 5 APK確認1時の追加所見）→ **ネイティブStackヘッダー計8画面はWeb修正済み（2026-07-14「画面文言・Webヘッダー修正」バッチ＋同日追加）**。原因の切り分け結果：視認できない/描画されないのは**ネイティブStackヘッダーの戻るボタン**（React Navigation Web既定のマスク画像シェブロン。ブラウザによって表示されず、履歴のない直アクセス時はそもそも描画されない）で、画面内 `AppHeader` のテキスト「← 戻る」（ブランド色）はもともと視認できていた。テキスト「← 戻る」の専用 `headerLeft`（`src/components/NativeHeaderBackButton.tsx`。履歴があれば `back()`、直アクセス時は fallback へ `router.replace`）を、まず当初3画面（`/memo/create`→ホーム・`/notes/create`→`/notes`・`/settings`→ホーム）に設定。**同日、同じ問題が確認された現場適応5入力画面（`/workplace/start`・`/stuck`・`/question`・`/report`・`/end`。fallbackはいずれも `/workplace`）にも追加適用し、`headerLeft` 対象は計8画面**。`/workplace/start?fromRestart=1` はクエリ・初期入力・引き継ぎ・保存/コピー処理を変更せず、履歴ありの戻るは従来どおり前画面へ。共通 `screenOptions` は無変更。**その後バッチ6A補足（2026-07-14）で、詳細・編集4画面（`/memo/[id]`・`/memo/[id]/edit`・`/notes/[id]`・`/notes/[id]/edit`）も同じネイティブStackヘッダーのマスク画像戻る（履歴あり視認性不安定・直アクセス時未描画＝行き止まり）を再現したため、同じ `NativeHeaderBackButton` を適用し `headerLeft` 対象は計12画面**。詳細のfallbackは静的（`/memo/[id]`→ホーム、`/notes/[id]`→`/notes`）、編集のfallbackは対応する詳細への動的ID（`/memo/[id]/edit`→`/memo/${id}`、`/notes/[id]/edit`→`/notes/${id}`）で、詳細＝`_layout.tsx` の `headerLeft`、編集＝各画面内の `<Stack.Screen options>`＋`useLocalSearchParams` で実装。タイトル（メモ詳細／メモ編集）は不変。メモ詳細・メモ編集の読み込み例外時にローディングが終わらない可能性は当時本補足では変更せず後続へ回したが、**バッチ6B-2（2026-07-15）で対応済み**（`app/memo/create.tsx`・`app/memo/[id]/index.tsx`・`app/memo/[id]/edit.tsx` に保存失敗の `showMessage` 通知＋読み込みの loading/load-error（再試行）/not-found 区別を追加。`ListStateView` 再利用。`memoRepository`・`memoService`・保存/取得本体は無変更。編集の動的戻るボタンは全状態で維持。テスト時に `getMemoById`/`runAsync` を一時差し替えて実例外を再現し確認。`30` §8.6.2）。Web確認：当初3画面＝6幅62項目（`30` §8.5.1）、現場適応5画面＝133項目（`30` §8.5.2）、詳細・編集4画面＝53項目（`30` §8.5.3。実データ作成→履歴あり/直アクセス/6幅/編集fallbackの正しいID/二重ヘッダーなし、スクリーンショット目視）。**Android実機・TalkBack・文字サイズ最大・HEADER-02全体は未確認のまま完了扱いにしない**（VISUAL／A11Y／Gate 6で最終確認）
* **FlowDockメモ作成画面の下部ボタン余白**（2026-07-13 実機確認時の追加所見）→ **実装対応済み（2026-07-15 最終統合バッチ）／Android実機確認は未実施**。`src/components/FormFooterBar.tsx` に `useSafeAreaInsets()` を導入し、**通常フッターのみ** `paddingBottom = 12 + insets.bottom` を加算（さくっとメモ作成/編集・記録作成/編集の共通フッターへ適用）。iOSの `InputAccessoryView`（キーボード直上バー）へは下部インセットを加算しない（重複回避）。Webや下部インセットのない端末は `insets.bottom=0` で従来表示に一致。`InputAccessoryView`/`InteractionManager`/`onAccessoryReady` フォーカス処理は無変更（`30` §8.7）。ヘッドレスChromeで作成/編集画面（`/memo/create`・`/notes/create`）が6幅で横スクロールなしを確認（Webは `insets.bottom=0` で従来表示に一致）。**Android実機での下端余白・下部インセット加算・キーボード表示時の見え方は最終APKで確認**（Webでは下部インセットが0のため差分が観測できない）
* **保存・コピー中の二重操作防止と結果タイマー**（Phase 15仕上げ項目）→ **実装対応済み（2026-07-15 最終統合バッチ）／対話的Web確認は未実施**。コピーは共通hook `src/hooks/useCopyFeedback.ts`（copying中は再実行無視＋`disabled`／`accessibilityState.disabled`、結果タイマーID保持で古いタイマー解除・アンマウント時解除・アンマウント後setStateなし）へ集約〔memo詳細本文・notes詳細本文/プロンプト・NoteFormプロンプト・WorkplaceSceneformコピー〕。現場適応の保存は `saveState='saving'`＋`savingRef` で `await onSave` 中disabled・二重実行なし（失敗タイマーもref保持・アンマウント解除）。プロンプト集の `copyingId` 方式は無変更（`30` §8.7）。ヘッドレスChromeで**現場適応の保存連続クリック→保存ボタン `aria-disabled=true`（二重実行なし）**・**コピー連続クリック非クラッシュ**・**コピー失敗時に「コピーできませんでした」表示＋約2.5秒後に既定ラベルへ戻る（タイマー解除）**を実確認。コピー成功中disabledの成功パスはヘッドレスの `clipboard.writeText` 拒否で未再現（通常ブラウザ/実機で確認）。**Web全体回帰（`30` §8.8）で追加確認：さくっとメモ/現場適応/記録の保存失敗→入力維持→再保存成功、記録詳細/編集の読み込み失敗＋再試行、さくっとメモ削除、Web戻る12画面×（直アクセス＋再読み込み）＝24件fallback正常、ページズーム1.5で横スクロール/戻る・タイトル重なりなし、console.error/warn/pageerror/unhandledrejection いずれも0件**
* **現場適応フォームのキーボードスクロール** → **解決（2026-07-15 Expo Go実機で合格）**：現場適応の入力画面（`WorkplaceSceneForm`）でAndroidキーボード表示中に下側入力欄が隠れ下まで十分スクロールできない問題に対し、**共通コンポーネント1か所**（`src/components/WorkplaceSceneForm.tsx`）で `Keyboard` の show/hide を購読し、表示中だけ `ScrollView` の下部余白へキーボード高さを加算（`paddingBottom: 40 + keyboardHeight`・非表示時40・二重加算なし）。当初 `keyboardDismissMode="on-drag"` を併用したが、**versionCode 8 APK実機でドラッグ開始時にキーボードが閉じ→追加余白も消えてスクロールできない副作用が判明**したため、**`keyboardDismissMode="none"` へ変更**（`30` §8.10）。`KeyboardAvoidingView` 置換・`app.json` 変更・画面別重複実装はしていない。ユーザーがPixelのExpo Go（LAN・専用ポート8115）で `/workplace/stuck` を確認し、**キーボードを表示したままスクロール可・スクロール開始でキーボードが閉じない・下側入力欄と「整理する」まで到達・入力維持・キーボード非表示後の余白復帰＝すべて合格**。tsc/Web export/Web回帰も合格。発表デモに影響するキーボード問題は解決確認済み。versionCode 9 の最終APKへ反映予定（`30` §8.10）
* **端末最大フォントサイズでの下側見切れ**（2026-07-15 APK確認3＝versionCode 7 の実機確認で判明）：**Androidの最大フォントサイズにすると画面が縦方向へ引き伸ばされ、一部画面で下側が見切れる**。通常の文字サイズ（発表で使用）では支障なし。発表には支障がないため今回は修正せず、**発表後の改善課題として保留**（Phase 15後。合格・完了扱いにしない＝A11Y-04・Gate 6は未通過のまま）。対象画面の特定・原因（固定高さ／`ScrollView` 外の要素／`minHeight` とコンテンツ拡大の関係等）の切り分け・修正方針は未確定。記録先：`30` §12.1（APK確認3）・§15（Gate 6/7）・`10` A11Y-04
* **TalkBack（スクリーンリーダー）確認**：APK確認3時点でも未実施＝**発表後へ延期**。付与済みの `accessibilityRole`/`accessibilityLabel`/`accessibilityState`/`accessibilityLiveRegion`（`30` §8.7）の実機読み上げ・フォーカス順序・操作可否は未確認。Gate 6の通過条件に残る（`10` A11Y-02）
* **コピー成功経路の実機確認**：APK確認3時点で明確な確認報告がないため**未確認のまま維持**（勝手に合格にしない）。「コピーしました」表示・クリップボード実内容一致を通常ブラウザまたは実機で確認する（失敗表示・二重操作防止・タイマー復帰・コンソールエラーなしは `30` §8.7・§8.8で確認済み）。GitHub連携の実通信も未確認＝完成扱いにしない
