...
* **現場適応フォームのキーボード下部余白**（2026-07-15 Android実機不具合。局所修正実装済み・Android未確認）：現場適応の入力画面（`WorkplaceSceneForm`）でAndroidキーボード表示中に下側入力欄が隠れ下まで十分スクロールできない（特に `/workplace/stuck` の後半フィールド）問題に対し、**共通コンポーネント1か所**（`src/components/WorkplaceSceneForm.tsx`）で `Keyboard` の show/hide を購読し、表示中だけ `ScrollView` の下部余白へキーボード高さを加算（`paddingBottom: 40 + keyboardHeight`・非表示時40・二重加算なし）＋`keyboardDismissMode="on-drag"` を追加。`KeyboardAvoidingView` 置換・`app.json` 変更・画面別重複実装はしない。tsc/Web export/Web回帰（5画面21項目・console/pageerror/unhandledrejection 0）合格。**Android実機／エミュレータでのキーボード回避動作（必須確認1〜10）は本セッションで未確認**（キーボードイベントはネイティブのみ発火。最終Android実機確認で確認する。`30` §8.9）
