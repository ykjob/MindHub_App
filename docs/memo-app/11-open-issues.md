...
* **ホームの設定導線の具体配置**：→ **確定（2026-07-14 バッチ2実装）**。AppHeader右側の小さな「設定」ボタンを採用。共有AppHeader本体は無変更（既存 `right` propへ `app/index.tsx` から注入する最小構成）。幅360〜1024のWeb確認でタイトル・補助文と衝突なし・`/settings` 遷移動作を確認
...
* **ホームFABと「すぐメモする」カードの役割重複**（2026-07-14 IA判断時に整理）：既存FABはスクロール位置にかかわらず軽量メモ作成へ進める即時入力導線として維持し、バッチ2では削除しない。カード＝機能の発見性、FAB＝即時操作性として役割を分け、扱いは実機確認後に評価する（`28` §7.2）。バッチ2実装後のWeb確認（2026-07-14）では、全件展開時もFABが操作可能で「すべて表示／3件に戻す」を覆わないことを確認済み。最終評価はAndroid実機確認後（保留のまま）
