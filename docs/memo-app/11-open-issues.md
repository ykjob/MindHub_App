...
* **ホームの実装着手時期**：基準確認は完了（上記）。ホームの本格再構成（主要機能カード化）は IA系判断（下記）の確定後。状態：確認待ち
* **最近のメモの取得元**：memos（軽量メモ）／notes（メモ管理）／両方のどれにするか（`28` §17-2）。決定時期：HOME-01着手前。状態：ユーザー判断待ち
* **AppHeaderの具体的な高さ・padding**：実機基準確認は完了。AppHeader実装タスク内で調整可能な値として扱う（`29` §12）

実装を止めないが、該当タスク着手前に確定が必要なもの。

* **現場適応モードの短い画面表示名**（候補：仕事サポート等。正式説明名は「現場適応モード」のまま。`28` §17-3）。決定時期：ホーム主要機能カードの文言確定時（HOME-02／IA-02）
* **プロンプト集の状況別分類**（6候補の採否・粒度。`28` §10.1）。決定時期：PROMPT-01着手前
