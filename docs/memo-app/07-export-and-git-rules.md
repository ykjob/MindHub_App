...
## 15. Markdownと完全バックアップ・GitHub取り込みの境界（2026-07-27 追記。将来方針）

バックアップ・復元・外部取り込み仕様（`34-backup-restore-and-external-import.md`）との役割分担を定める。正本は `34`。現時点ではいずれも未実装（将来候補・実装時期未定）。

* Markdownは、人が読むこと・Gitで文章の履歴を管理すること・文章を共有することを目的とする形式である
* MindHub全体の完全バックアップ・復元の正式形式はJSONとする（Markdownは完全バックアップに使わない）
* GitHub上のMarkdownからの取り込みは、将来の**手動・個別**の取り込み（主にさくっとメモの個別復元・外部文章からの新規作成補助）であり、自動同期・共同編集・削除同期ではない
* 現場適応モードのGit除外・守秘方針（§13・`23`）は維持する。現場情報はGitHub取り込み・公開の対象にしない
