...
公開判定はカテゴリ（type）だけで行わず、visibilityを併用する。type = family でも visibility = private / family のものは公開GitHub Pagesに出さない。visibility = family 値の実装タイミングは `11-open-issues.md` 13章参照。
