...
## 9. バックアップ・復元・外部取り込みのID方針（参照。2026-07-27追記。将来方針）

完全JSONバックアップ・復元・GitHub Markdown取り込み・共有コピー（`34-backup-restore-and-external-import.md`）に向けた、IDに関する将来方針のみを記録する。現時点ではカラム・テーブルを追加しない。正本は `34`。

* `memos.id` / `notes.id` は既存のUUIDとして維持する
* IDには、ユーザー・端末・リポジトリ・ワークスペース・会社・所属などの情報を埋め込まない（必要になった場合も別項目として扱う）
* 外部ファイル上のIDとローカルIDの分離が必要になった場合でも、既存IDは変更せず、`§6.5` の schema_version 方式で外部参照用の別テーブルをマイグレーションで追加する
* 具体的な外部参照テーブル名・カラム・導入時期は確定しない（`34` §8.4 は例示であり作成済み・確定済みではない）
* 現時点ではカラム追加・テーブル追加・schema version 変更を行わない
