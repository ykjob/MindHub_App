...
2026-07-12追記：開発リファレンスの目的は2本柱（柱1＝技術リファレンスR-01〜R-31、柱2＝現場理解カードG-01〜G-20。`24` §1）に拡張したが、どちらも同じ `command` カテゴリ＋タグ `dev-ref` 運用のまま扱う。現場理解カードは第2タグとしてサブ観点タグ6種（`onboarding` / `ask-report` / `troubleshoot` / `git-db` / `security` / `closing`。`25` §10.3）を併記する。新type・新カテゴリの追加はしない。
