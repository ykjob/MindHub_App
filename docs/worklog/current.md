...
### PR #5 merge前の現在状態整合修正

GitHub上での最終確認で、`current-tasks.md` の**Phase 16C最終完了項目の内部に旧文「Phase 16Cは最終完了ではない」が残っている**ことを検出したため削除した。あわせて、同項目の後半に残っていた**旧レビュー時点の詳細**（再々レビュー結果の要約・`78625c4`／`c1828b5` の検証時系列・「今回（2026-08-05）の修正もtscを再実行していない」等）も、現在状態の説明として不要なため `詳細は …docs/worklog/current.md。` の参照以降から整理した（同内容は下の過去エントリ・`33` §21〜§23・本ファイルに履歴として残っている）。また `10-tasks.md` §21の状態日付を **2026-08-05時点 → 2026-08-08時点** へ修正した（本文はすでにv10最終受入・Gate PASS・Phase 16C完了を記録済み）。

**Gate判定・K1〜K22の結果・v10ビルド結果そのものは変更していない**。`32`・`33`・`11-open-issues.md`・`09-roadmap.md`・`AGENTS.md`・`CLAUDE.md`・`00_START_HERE.md` も無変更。**コード・設定・DB・依存・versionCode（10のまま）は変更なし／EAS再ビルド・Expo Go・Pixel等の再テストなし**。**PR #5 はmerge前**。

## Phase 16C：Expo Go実機事前確認PASS（2026-08-05）

対象main：`b96cc669ddc7c33f3563070d85962a07bb3b85fd`（PR #3 merge commit）。**Pixel＋Expo Go（SDK 54）**で実施。Metro：`exp://192.168.0.225:8118`（ポート8118・LANモード）。
...
* **Q1〜Q16（現場適応・質問）：全合格**
* **R1〜R14（現場適応・報告）：全合格**
* **D1〜D10（Android共有横断）：確認範囲は合格**（OS共有起動・ChatGPT選択・自動送信なし・キャンセル後復帰・日本語/改行/長文保持・多重共有なし・クラッシュなし・誤った完了表示なし）
* **不具合・クラッシュ・重大な表示崩れなし**
* **未確認：ChatGPT未インストール端末**（確認のためのアンインストールは行わず、Android標準OS共有を使う実装であることはコードレビュー済み。この1項目は事前確認PASSを妨げない）
* Metroは確認後に**停止済み**（ポート8118は未LISTEN、Metroのnodeプロセスは終了）

### 状態

* **コード・設定・依存・versionCodeの変更なし**
* **EAS build未実施／APK未更新**
* **`33` Gate D・`32` Gate D は最終未通過**（最終判定は更新APKでのPixel確認後）
* **Phase 16Cは最終完了前**
* 次は **EAS APK更新**（ユーザー承認後）
...
### PR #4 Codexレビュー指摘対応：Android欄の状態整理

Codexレビューで、`docs/memo-app/10-tasks.md` の 16C-1〜16C-3 **直下のAndroid実機確認欄に旧状態が残っている**と指摘された。大項目は「Expo Go事前確認PASS（2026-08-05）・EAS APK更新待ち」へ更新済みだったが、直下が「**＝未確認**」「Expo Go未実施」のままで矛盾していた。

3項目を、**Expo Go事前確認済み・合格**と、**更新APKで行うGate D最終確認待ち**が区別できる表現へ更新した。

* 16C-1：OS共有起動・キャンセル後復帰・日本語/改行/長文保持・多重起動なし・クラッシュなしを確認済み。**ChatGPT未インストール端末は未確認**。更新APK未作成のため `33` Gate Dの最終判定は更新APKでのPixel確認待ち
* 16C-2：プロンプト集・さくっとメモからの共有画面起動、ChatGPT選択、自動送信なし、キャンセル後復帰、日本語/改行/長文・編集文章の維持、多重起動なしを確認済み。更新APKでのPixel最終確認待ち
* 16C-3：質問・報告の共有、守秘3チェック制御、本文変更後の全解除と再確認、キャンセル後の文章維持、private・Git候補false・専用タグでの保存、二重保存なし、誤った完了表示なしを確認済み。`32`・`33` Gate Dの最終判定は更新APKでのPixel確認待ち（Gate EはWeb確認済み）

**16C-1〜16C-3 の `[ ]` は維持**。**`32` Gate D・`33` Gate D の `[ ]` も維持**。`current-tasks.md`・`32`・`33` は**変更なし**。コード・DB・依存・設定・versionCode も**変更なし**。**EAS build・APK作成なし**。**PR #4 はmerge前**、**Phase 16Cは最終完了前**。

---

（以下は過去の記録）

最終更新（前回）：2026-08-05（**Codex最終文書再確認＝PASS**（Critical/High/Medium/Low/Note すべて0）。製品コードと文書のレビュー工程は完了し、**PR作成・main反映へ進める状態**。ただし**Expo Go事前確認・EAS APK更新・更新APKでのPixel最終確認は未実施**で、**Phase 16Cは最終完了ではない**）。前回：2026-08-05（最終文書再確認で残った文書Low 2件を修正）、2026-08-02（最終文書確認で残った文書Low 2件を修正）、2026-08-02（再々レビューで残った文書Low 1件を修正）、2026-08-02（Codex再レビュー指摘 High 1・Low 1 を修正）、2026-08-02（**Phase 16C を実装**）ほか
...
### 目的
ユーザーが配置した新規構想文書 `docs/memo-app/35-quick-memo-brain-dump-and-ai-handoff.md`（構想の追加日は文書本文どおり2026-07-30）を、**未承認・未実装の構想**として文書体系へ整備した。35は音声ブレインダンプ／整理待ち／本人整理欄／用途別プロンプト編集／課金境界／AI結果保存にまたがる横断構想であり、Phase 16Cを置き換えない・33へ統合しない・**新Phase番号を作らない**。今回（2026-08-02）はレビュー結果の反映のみで、コード・DB・依存・Phase 16Cは変更していない。

### 35で修正した箇所
* 冒頭に状態表示を強調（未承認の構想／未実装／Phase 16Cの正式仕様ではない／承認済みは31・32・33／35だけを根拠に着手しない／新Phase番号を作らない）
* §1.1「関連正本と責務分担」を新設（入力=`31`／共有=`33`／テンプレDB=`12`＝Phase 8〜9／プロンプト集=`14`／バックアップ=`34`。再定義しない・二重実装しない・共有ボタン名は`33`の「ChatGPTなどへ共有」に合わせる）
* §5.1 に入力正本=`31` を明記／§5.5 に共有正本=`33` を明記／§6 冒頭に `12`・Phase 8〜9 との関係と「構想追加を理由にDB・migrationを変更しない」を明記／§12.1 に `34`（バックアップ）との関係を明記
* §5.4 に4用途（構想候補）・`33`の5用途・`14`の6入口の未統合を明示／§18 暫定結論を「未承認の暫定判断」と明示
* 35固有の構想（音声ブレインダンプ思想・整理待ち・本人整理欄・用途別プロンプト編集・標準/編集値分離・標準復元・`{{本文}}`変数・カスタムボタン・課金境界・AI結果保存・原文/本人整理/AI案/確定内容の分離・利用状況記録）は内容を再設計せず維持

### 参照追加
`00_START_HERE.md`（関連文書一覧）・`CLAUDE.md`／`AGENTS.md`（読み分け表に同一趣旨の行）・`09-roadmap.md`（Phase 13 将来拡張候補・新Phase番号なし）・`10-tasks.md`（§15.2 将来候補・16Cタスクへ混ぜない）・`11-open-issues.md`（§19 主な未決定のみ・全項目は`35`§15参照）・`current-tasks.md`・本ファイル。

### 無変更・検証
`app/**`・`src/**`・`app.json`・`package.json`・`package-lock.json`・`eas.json`・DB・migration・既存ルート・Phase番号・31/33/34の既存決定内容は無変更。Phase 16C未開始。文書のみのため tsc／expo export／expo-doctor は非実行。35内に「クイックメモ」「利用者向けFlowDock」「31の誤ファイル名」「Phase 16C実装済み表現」「35を根拠に新Phase番号を作る記述」がないことを確認。

### Codex初回レビューと修正（2026-08-02）
体系整備コミット（`2b34615`）に対するCodex初回レビュー＝**要修正・再レビュー**。集計：Critical 0／High 1／Medium 2／Low 1。今回 High・Medium を修正した。

* **High 1**：`33`（Phase 16C）で確定済みの共有確認画面を、35 §15で「ボタン押下で即時コピーするか、必ず確認画面を挟むか」と未決定へ戻していた。→ §5.5 を「用途別生成文章は必ず `33` の共有確認画面を経由し、全文確認・編集後にコピー／OS共有する（即時共有・即時コピーしない）」と明記し、§15 の当該未決定項目を削除（未決定として残したのは共有確認画面へ進むボタン配置・一時状態保持・既存本文コピーとのUI並び等のUI細部のみ）。既存「本文をコピー」は元本文の直接コピーの別導線として維持する旨も明記
* **Medium 2**：(a) 5用途の節参照誤り `33 §8.2`（実際は全文入力欄）→ 正しい `33 §7.2 SHARE-MEMO-02` に修正（35 §5.4・`11` §19）。(b) 35冒頭 line 3〜6 の行末半角空白（Markdown hard break）4件を除去し、各メタ行を空行区切りの段落へ（LF維持・追加日2026-07-30維持・内容不変）
* **Low 1**：`.claude/settings.local.json` は未追跡のローカル設定ファイル。**対象コミットには含まれない。今回は変更・削除・stage・commit していない**（`.gitignore`・`.git/info/exclude` も変更しない）

修正後 `git diff --check` 問題なし。（初回レビュー分の修正後の状態。以降は下記の再レビュー対応を参照）

### Codex再レビューと修正（2026-08-02）
初回指摘修正後のコミット（`7f1270d`）に対するCodex再レビュー＝**要修正・再レビュー**。集計：Critical 0／High 0／Medium 1／Low 0／Note 1。今回 Medium・Note を修正した。

* **Medium 1**：35 §15 に「用途選択と共有確認画面を同一画面にするか分けるか」を未決定として残していたが、`33` §7.2 SHARE-MEMO-02・§8.1 により**用途選択は共有確認画面内で行うことが確定済み**（共有確認画面の表示項目に用途選択を含む）。→ §15 から当該記述を削除し、未決定として残すのは「共有確認画面内の用途選択UI配置・共有確認画面へ進むボタン配置・一時状態の保持・既存本文コピーとの並び・プロンプト編集の操作位置」等の**33が決めていないUI細部のみ**に限定。§5.4 にも「用途選択画面を共有確認画面と別に設けない／最終的な用途選択・全文確認・編集は `33` の共有確認画面内（§7.2・§8.1）」を明記。**35は `33` の共有画面構成を再定義しない。**
* **Note 1**：`.claude/settings.local.json` の記録を実測に合わせ、「ユーザー全体の global ignore により無視」という断定を削除。**未追跡のローカル設定・対象コミット外・今回は変更/stage/commitしていない**、とのみ記載（`.gitignore`・`.git/info/exclude` は変更しない）。

修正後 `git diff --check` 問題なし。（再レビュー分の修正後の状態。以降は下記の再々レビュー結果を参照）

### Codex再々レビュー結果（2026-08-02）PASS
再レビュー指摘修正後のコミット `c9273646bcaeb820a7e5d8bf2e49dc9a20b19f2a`（`docs: add quick memo AI handoff concept`）に対するCodex再々レビュー＝**PASS**。集計：Critical 0／High 0／Medium 0／Low 0／Note 0。初回レビュー・再レビューの指摘（High1／Medium2／Low1／Medium1／Note1）はすべて解消済み。

* 承認対象コミットは**文書9ファイルのみ**（`00_START_HERE.md`・`AGENTS.md`・`CLAUDE.md`・`current-tasks.md`・`docs/memo-app/09-roadmap.md`・`10-tasks.md`・`11-open-issues.md`・`35-quick-memo-brain-dump-and-ai-handoff.md`（新規）・`docs/worklog/current.md`）。`git diff --check` clean。
* コード・DB・依存・app.json・eas.json・schema_version・migration・既存ルートは無変更。Phase 16C未開始・新Phase番号なし。35番は未承認・未実装の将来構想のまま。
* `.claude/settings.local.json` は未追跡のローカル設定で、対象コミット外（変更・stage・commitしていない）。
* この管理文書更新（本記録）は、承認済みコミット `c927364` を amend せず**別コミット**として作成し、作業ブランチをGitHubへpushする。**push後はPR作成待ち**（PR作成・mainへのmerge・ブランチ削除は行わない）。
...
* `app/settings.tsx`：GitHub Repository 入力欄の直下に補足文を追加（プレースホルダ `例: FlowDock_Notes` は既存privateリポジトリ名の例示のため維持。補足＝「例は書式の目安。既にアップロード運用中なら現在のリポジトリ名をそのまま入力。名称整理のために作り直す必要はない」）。既定値と誤認させない・所有者名や実リポジトリ名を新たに表示しない・GitHub設定/保存値/設定キー/アップロード処理は無変更。ホーム（`app/index.tsx`）と Web document title（`app/_layout.tsx`）は Phase 15/16A で既に移行済み＝追加のコード変更なし

### 文書の変更（旧名称方針が現在の規範として読めない状態にする）
* `01` §1.6・§1.7：見出し直下に「2026-07-12／2026-07-13 当時の歴史的記録・現在は §1.10 優先」と明示。過去本文は改変せず、優先関係を読み始めで示す
* `01` §1.10：現在の名称境界の正本（初回で新設。今回維持）
* `28`（UI・UX）：§11 と §6.1・課題表 UX-04・§14 非対象・§17 実装前決定#6・§19 参照を §1.10 基準へ更新（app.json name は変更済み・新規表示で FlowDock 不使用・app.json 変更「時期未確定」を解消）
* `11`（未解決事項）：§1 技術構成の内部名記述・「表示名はFlowDockのまま変更しない」・「app.json 表示名を変更する時期」を **決定済み・§1.10 参照** へ更新（過去の未確定は歴史記録として残す）
* `08`（UIフロー）§1：ホーム導線の「クイックメモ」→「さくっとメモ」（`/memo` 系＝FlowDock由来の内部注記は残す）
* `30`（検証チェックリスト）§8.1・§8.5：現在の検証項目の「クイックメモ」→「さくっとメモ」、FlowDock を現在UIへ表示してよいと読める条件を「MindHub／さくっとメモで統一・新規表示に FlowDock を出さない」へ更新。歴史的検証結果は当時の記録として保持
* `CLAUDE.md`・`AGENTS.md`・`00_START_HERE.md`：初回で §1.10 へ更新済み（今回追加変更なし）
* `10-tasks.md` §18.1・`current-tasks.md`・本ファイル：作業状態を更新

### 維持（名称だけを理由に変更しない。詳細は §1.10）
DB名 `flowdock.db`、旧Markdownの `source: FlowDock`、`FlowDock_Notes` プレースホルダ、`slug` / `scheme`（`flowdock`）・`android.package`・`projectId`・`versionCode`、npm名 `flowdock`、`scripts` の `flowdock.db` 参照、由来コメント「（旧FlowDock）」、歴史的仕様書・作業ログ。

### 名称検索（大文字小文字を区別しない）
* 対象語：FlowDock / flowdock / FLOWDOCK / MindHub / さくっとメモ / 軽量メモ / クイックメモ。除外：node_modules・dist・.git・生成物
* `flowdock`（大小無視。`FlowDock`/`flowdock`/`FLOWDOCK` を含む）件数の比較（同一手法＝`git grep`、追跡ファイル対象。node_modules/dist/.git は未追跡＝対象外）：
  * **変更前 base `68c5e130`：34 ファイル / 151 matching lines / 163 occurrences**（Codex報告の「151箇所」は matching lines に相当）
  * **修正後（本コミットの最終状態）：34 ファイル / 186 matching lines / 232 occurrences**
  * 増加分は本作業の記録（本ログ・タスク文書・仕様書に FlowDock を説明・注記として追記したこと）による。ファイル数は不変。利用者向けの現在表示に残る FlowDock は無し（後述の分類 10・11 は 0 件）
...
### 目的
ユーザーが配置済みの新規正本 `docs/memo-app/34-backup-restore-and-external-import.md`（将来候補・実装時期未定・現在Phase対象外）を基準に、既存仕様書・ロードマップ・管理文書へ参照と必要最小限の要約を反映する。実装・DB・依存・versionCodeは変更しない。

### 更新文書一覧
`00_START_HERE.md`（参照先追加）／`docs/memo-app/01`（§1.9 設計境界）／`03`（§9 IDの将来方針）／`07`（§15 Markdown・Git・取り込みの境界）／`09`（Phase 13「データ可搬性・復元」）／`10`（§15.1 将来候補の親タスク）／`11`（§18 決定済み／未確定の要約）／`15`（§1.11〜1.14 将来候補）／`16`（§9 18と34の分担）／`17`（§6.3 共有コピーの境界）／`18`（§1.1 34との関係・§5 重複ID改定・§6 競合件数）／`docs/flowdock_mvp_design.md`（§6.1.1 後続仕様注記）／`current-tasks.md`／本ファイル。

### 決定した境界
完全バックアップはJSON／Markdownは閲覧・共有・個別取り込み用／`memos`・`notes`の正式復元はJSON／復元は元ID維持・共有コピーは新ID／IDへユーザー・端末・会社・所属情報を埋め込まない／同じIDで内容が異なっても自動で別メモと判定しない・`updated_at`だけで自動上書きしない／スマホ左右2列比較なし・初期版は自動マージなし／自動同期・共同編集は別の将来機能（Supabase同期候補ともさらに別）。

### 18の重複ID・自動上書きの変更
§5を改定し、空の環境への復元は元IDを維持して追加、既存環境への統合は同ID同内容スキップ・同ID内容相違は競合、`updated_at`だけで自動上書きしない・明示選択なしに既存データを置換しない扱いへ変更。§6の結果表示に「競合：n件」を追加し、旧「更新：n件（`updated_at`自動上書き）」を廃止。JSON例・notes全カラム方針は矛盾しない範囲で維持し、完全バックアップJSONの最終形式は18で確定しない。

### 未確定事項
`11` §18へ集約（JSON：画面配置・Androidファイル選択・具体形式・トランザクション・暗号化/世代管理／GitHub Markdown：一覧・内容取得・外部参照テーブル導入時期・差分表示／共有：`originRecordId`導入時期・共有コピー用JSON・共同編集条件）。

### 検証結果
`git diff --check` 問題なし。文書のみのため型チェック・Expoビルドは実施せず（コード・DB・依存・app.json・eas.json・versionCode変更なし）。34自体は原則変更していない。想定外の既存変更なし。**commit・push完了：`4bd0839c990727fd730a4ca9ad0300884e42b731`**。

### 変更対象外（守った）
`docs/memo-app/02-features.md`・`08-ui-flow.md`・`docs/architecture.md`・`src/**`・`app/**`・`package.json`・`package-lock.json`・`app.json`・`eas.json` は無変更。
...
再レビューで残った3点だけを文書修正した。新しい製品判断は追加せず、`31`〜`33` の承認済み製品判断も変更していない。実装は未着手。

* **`09-roadmap.md`**：Phase 16Bの完了条件「Gate A〜E」を、16B単体は **Gate A〜C・E**、Gate D（共有・守秘）は16C-1／16C-3統合時に確認する連携Gートで**Gate D未通過だけを理由に16B-1〜16B-3を未完了にしない**、へ修正（同節の連携Gate定義と整合）
* **`32-workplace-question-timing-rules.md`**：§8の `詰まり記録または質問画面には、判定せず質問文を作る導線を置く。` を `質問タイミング確認画面には、判定せず質問文を作る導線を置く。` へ修正。承認済みの「詰まり記録→質問タイミング確認→質問文作成」の配置は不変。現場適応入口から開く独立機能も追加しない
* **`11-open-issues.md`**：§17を未決定事項の参照のみへ整理。保存後の再編集に関する未決定事項は **§14を正本**とし§17へ全文重複しない。QMEMO-01〜14の実装・検証状態は未決定の製品判断ではないため§17から削除し `31` §16・`10` §21へ集約。解消済みレビュー履歴は本ログに残しopen issuesへ掲載しない。Phase 16の確定仕様は `01`・`31`〜`33` を正本とする記述は維持

無変更・非対象：`app/**`・`src/**`・`scripts/**`・DB・設定・依存関係。`.claude/`・`AGENTS.md` は変更・追加・削除・ステージしていない。commit・push未実施。**次の作業＝再レビュー**。

---



## Phase 16文書統合レビューの反映（2026-07-23、文書のみ・未コミット）

### 今回の作業

Phase 16文書統合差分のレビューで、新規仕様書 `31`〜`33` と既存仕様書の間に残っていた不整合が見つかったため、**文書だけ**を修正した。新しい製品判断は追加せず、`31`〜`33` の承認済み要件も再設計していない。実装は未着手。

### 見つかった不整合と修正

* **守秘・保存条件の競合**（`23`・`11`）：`23` §5.1が質問・報告保存で private・Git候補false を強制する一方、§6.1が「ユーザーが個別に変更することは妨げない」としていた。→ §5.1でPhase 16の質問・報告の**新規保存**は private・Git候補false を強制し保存画面で変更させないと明記。§6.1.1を新設し、一般メモの既定（変更可）とPhase 16の新規保存（強制）を区別。**保存後の再編集時に変更可能にするかは今回未決定**として `11` §14へ記録（永続ロック・変更禁止機能は勝手に採用しない）
* **正本の二重化**（`11`）：§17がPhase 16の確定事項を長く転載していた。→ §17を短縮し「確定仕様は `01`・`31`〜`33` を正本とする」の参照のみに。未決定事項だけを残し、「実質的な矛盾は未検出」の記述を削除。§14の未決定対象から質問・報告の保存可否と `workplace_question`/`workplace_report` タグを外し、再開メモ・その他場面メモ・作業開始/詰まり/終業前/現場タグだけを残した
* **16B・16Cのタスク重複**（`09`・`10`・`32`・`33`）：16B-3と16C-3が質問共有・守秘・保存を二重担当し、16B-3が未実装の16C-1へ依存していた。→ 16B-3=**完成質問文の任意保存のみ**、16C-3=**質問・報告の共有接続＋必須守秘チェック＋報告文の任意保存**へ分離。現場適応専用保存処理を共用し二重実装しない。16Bの判定・保存は16C共有基盤なしで実装可能とし、`32` Gate D（共有）は16C-1／16C-3統合時の連携Gateと明記（16B完了を16C未実装だけで不合格にしない）。`09`・`10`へ依存関係・実装順を明記
* **質問タイミング仕様の不整合**（`32`）：導線を「詰まり記録からの質問タイミング確認導線」に限定（現場適応入口からの独立機能は追加しない）。入力群を「5群」→「6群」へ修正（QT-INPUT-01〜06）。QT-RULE-01から「現場ルールで即時確認が求められている」の**自動判定を除外**（QT-INPUT-01は有無しか取得せずルール本文を持たないため）＝現場ルールありは「現場ルールを優先」注意表示に留める。Gate Dを「3つの守秘チェック完了前はコピー・共有できない」（`33` と同条件）へ修正
* **共通共有ルートの可否**（`01`・`CLAUDE.md`・`10`・`29`）：「ルートを変更しない」一括表現を分離。既存ルートの名称変更・削除・移動・構成破壊はしないが、Phase 16Cでは `33` §13 を満たす共通共有確認ルートの**追加は可**（route名は実装判断・共有に不要な別ルートは追加しない・URLクエリへ全文を入れない・Web直アクセスで漏えい/空白/クラッシュを防ぐ）と明記
* **Android対象範囲の矛盾**（`20`）：Android APK版が将来候補のまま残っていた。→ Phase 14当初のPC Web優先は歴史的記述として残し、**現行Phase 16ではAndroid APK版も実装・検証対象**（質問タイミング画面・OS共有をAndroidで確認）と明記。オフライン機能の新規拡張・端末間同期は将来候補のまま＝「Android利用」と「オフライン拡張」を同一視しない。§7の「生成テキストのコピー」にPhase 16Cでコピーを残しOS共有を追加する参照注記を追加
* **既存実装と未実装タスクの区別不足**（`31`・`10`）：`31` §16に「要件別実装状態」表を追加。QMEMO-08（保存状態）・iOS `InputAccessoryView`・`FormFooterBar` Safe Area・`multiline` 入力・保存中二重実行防止/ActivityIndicator/失敗時維持/再保存は**実装済み**＝Phase 16では維持・回帰確認。QMEMO-06/07は一部実装（Androidキーボード表示時・最終行可視性は未検証）、QMEMO-01〜05は一部実装/未検証、QMEMO-10〜14（ホーム領域分離）は未実装。`10` の16A-2に「既存保存処理を書き直さない・まず維持して検証・未達部分のみ修正・共通化で保存ロジックを書き換えない」を明記
* **参照追記漏れ**（`02`・`21`）：`02-features.md` 末尾にPhase 16参照節を追加（詳細転載なし・未実装明記）。`21` の「`22` §8.1・§7.2」を「`22` §8.1・本ファイル §7.2」へ修正

### 無変更・非対象

* `app/**`・`src/**`・`scripts/**`・DB・schema version・設定（`app.json`/`eas.json`/`package.json`/lockfile）・依存関係・ルート実装は無変更
* `31`〜`33` の承認済み製品判断は変更していない。新しい製品判断は追加していない
* `AGENTS.md`・`.claude/` は変更・追加・削除・ステージしていない
* 実装・EASビルド・APK作成なし。Phase 16を実装済みとは記録していない

### 停止条件

commit・push は未実施。差分レビュー待ちで停止。**次の作業＝再レビュー**。

---



## Phase 16仕様書追加・既存仕様への統合（2026-07-23、文書のみ・未コミット）

### 今回の作業

ユーザーとChatGPTが作成・承認済みのPhase 16新規仕様書3件を、既存仕様体系へ参照・要約・Phase・タスク・守秘境界として統合した。**新規仕様の再設計はせず、既存文書へ必要最小限の参照・要約・タスクを追記するのみ**。今回は文書更新のみで実装コードは変更していない。

### ユーザー承認済みの決定（今回変更・再提案しない）

* Phase 16構成：16A（さくっとメモ入力・ホーム操作性）／16B（質問タイミング判断支援）／16C（外部AI・共有受け渡し）
* コピー＋OS共有（`ChatGPTなどへ共有`）、自動送信・API連携なし、共有先はユーザーが選ぶ
* 質問タイミング：通常20分／完全停止10分／危険条件は即確認、現場の正式ルール優先、3段階判定、判定スキップ可、判定途中・結果は保存しない、完成質問文のみ任意保存
* 現場適応の外部共有は守秘チェック必須、通常系は注意文
* 初期対象4系統（プロンプト集・さくっとメモ詳細・現場適応の質問・報告）、さくっとメモは5用途

### 新規仕様書3件（全文を読んで統合。転載はしない）

* `docs/memo-app/31-quick-memo-usability-improvement.md`（Phase 16A）
* `docs/memo-app/32-workplace-question-timing-rules.md`（Phase 16B）
* `docs/memo-app/33-external-handoff-and-sharing.md`（Phase 16C）

### 既存文書への参照・要約追記

* `00_START_HERE.md`：Phase 16の読む文書（16A=31／16B=32＋20〜23／16C=33＋06＋23）
* `CLAUDE.md`：読み分け表にPhase 16行追加、実装ルールにPhase 16の追加ルール（31〜33を先に読む・DB/ルート/依存/守秘を勝手に変更しない・共有は自動送信でない・未決定は勝手に決めない・着手時はまず調査/計画/変更予定ファイル報告のみ）
* `docs/flowdock_mvp_design.md`：§4.4.1に16A/16C参照（長文入力・本文コピー維持・`ChatGPTなどへ共有`・保存形式/GitHub/削除は無変更）
* `01-decisions-and-scope.md`：§1.8にPhase 16の追加決定（採用・変更しないもの・将来候補/対象外との整合。OpenAI API連携は将来候補、直接受け取りは対象外と矛盾させない）
* `03-data-model.md`：§8にスキーマ変更なしの参照（16A memos無変更／16B判定途中・結果保存なし・完成質問文は既存notes／16C共有一時文章・履歴は自動保存なし）
* `04-categories-and-tags.md`：§1.2.1に質問=`workplace,workplace_question`／報告=`workplace,workplace_report`・type=thought・private・Git候補false・守秘既定優先を確定
* `06-chatgpt-prompt-copy.md`：§1.2にコピー維持＋OS共有導線追加・正本は33・本文正本は06/14・API/直接送信なし
* `07-export-and-git-rules.md`：§14にOS共有とGit/公開出力の境界（別機能・任意保存質問報告はGit候補false・守秘は23/33）
* `08-ui-flow.md`：§11に横断導線（16Aホーム固定領域／16B詰まり→判定→質問・スキップ／16C共有確認・守秘差）
* `09-roadmap.md`：Phase 15後にPhase 16（16A/16B/16C・各Gateを完了条件・未着手）
* `10-tasks.md`：§21にタスク16A-1〜16C-3（仕様書・変更対象候補・完了条件・Web確認・Android実機確認・回帰確認。すべて未着手）
* `11-open-issues.md`：§17にPhase 16の確定仕様は`01`・`31`〜`33`を正本とする旨を記録し未確定へ戻さない（※後述のレビュー反映で§17は短縮し、確定事項の転載と「実質的矛盾は未検出」記述を削除）
* `14-mobile-prompt-hub-and-inbox.md`：§1.9に既存42件・検索・状況別入口・コピー維持＋各プロンプトに共有・本文は選択プロンプト・生成HTMLへの共有ボタン追加は決めない
* `16-platform-and-distribution.md`：§8にプラットフォーム差（Android OS共有／Webは共有可否でブラウザ共有かコピー案内／iOSは必須外／実送信保証なし）。ビルド方式・SDK・配布区分は無変更
* `17-distribution-and-sharing.md`：§6.2にOS共有と三区分の区別（公開用データ生成でない・確認済み1件を外部へ渡す・現場適応は守秘優先）。三区分は緩めない
* `20-workplace-adaptation-overview.md`：§2.3に質問タイミング支援を目的へ追加（詰まり→質問の補助導線・5場面モデルは不変）
* `21-workplace-adaptation-flows-and-ui.md`：§7.2に詰まり→判定→質問・スキップ・コピー＋共有・質問/報告の任意保存・判定詳細32/共有詳細33（5場面・翌朝再開・終業前保存は不変）
* `22-workplace-adaptation-data-and-integration.md`：§8.1で「質問・報告はコピーのみ／保存可否未確定」を確定仕様（判定途中・結果は保存なし・完成質問文/報告文は既存notesへ任意保存・専用テーブルなし・private/Git候補false）へ置換（変更日・参照先明記）。§4の質問/報告タグを確定へ更新
* `23-workplace-adaptation-security-and-portfolio.md`：§5.1に外部共有時の必須3チェック（顧客/会社/個人名なし・認証/社内URL/内部システム名なし・会社固有手順/判断基準/非公開情報なし）・完了前はコピー/共有不可・保存はprivate/Git候補false・OS共有も外部送信・共有先管理は範囲外。既存保存禁止情報は弱めない
* `28-ui-ux-quality-improvement.md`：§18.1にPhase 15後の追加改善参照（16A=31/16B=32/16C=33）。UX-01〜16判定は無変更
* `29-ui-design-system.md`：§10.1にPhase 16共通ルール（長文編集欄と固定操作領域の非重複・コピーと共有は別操作・`ChatGPTなどへ共有`ラベル・共有状態idle/sharing/failed・送信完了と表現しない・守秘チェックの選択状態）。部品名/route名は確定仕様として書かない
* `30-ui-validation-checklist.md`：§19.1にPhase 16固有確認は31〜33で管理・本ファイルの幅別/A11Y基準を参照という境界注記のみ。Phase 15のGate・判定・証跡は無変更
* `current-tasks.md`：現在フェーズ先頭にPhase 16文書統合作業を記録

### 無変更（今回触っていない）

* DB・schema version・コード（`app/**`・`src/**`・`scripts/**`）・既存ルート・依存関係・`package.json`/lockfile・`app.json`・`eas.json`・SDK
* プロンプト本文・Phase 15の検証結果/証跡/合否・新規仕様書31〜33の要件内容
* 実装・EASビルド・APK作成なし。未確認事項を実施済み扱いにしていない

### 停止条件

commit・push は未実施。差分レビュー待ちで停止。
...
* versionCode 7・対象コミット `78aeba8`・build ID `e0352c88-4440-4117-a446-568c9e4cf96e`・profile `preview`（internal apk）・SDK 54
...
### ユーザーによる実機確認結果（推測せず記録）

* **確認済み（発表可能）**：発表で使用する**通常の文字サイズではAndroid実機利用に支障なし**
* **既知課題（合格にしない）**：**最大フォントサイズにすると画面が縦方向へ引き伸ばされ、一部画面で下側が見切れる**。発表には支障がないため今回は修正せず、**発表後の改善課題として保留**（Phase 15後。`11` §16）。アクセシビリティ上の未完了課題として扱い、A11Y-04（文字拡大）は合格にしない
* **延期**：**TalkBackは今回確認せず、発表後へ延期**
* **未確認**：コピー**成功**経路（「コピーしました」表示・クリップボード実内容一致）は明確な確認報告がないため未確認のまま維持。GitHub連携の実通信も未確認＝完成扱いにしない
* **Gate 6・Gate 7**：TalkBack未確認＋最大フォントの既知見切れがあるため**完全合格・完了扱いにしない**。通常文字サイズでの発表可能性（確認済み）と、アクセシビリティ上の未完了課題（最大フォント見切れ・TalkBack）は分けて記載

### 記録先・非変更

* 記録：`30` §12.1（APK確認3新設）・§15（Gate 6/7を未完了と明記）・`28` §16・`10` §20・`11` §16・`current-tasks.md`・本ファイル
* **今回は文書のみ**。アプリ実装・UI・app.json・versionCode・eas.json・package.json・DB・依存・ルートは無変更。EAS再ビルド・APK再作成なし。最大フォント見切れの修正・TalkBack代行確認は行わない（Phase 15後）
