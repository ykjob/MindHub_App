...
* **Phase 16C 最終完了（2026-08-08、versionCode 10 APK最終受入PASS）**：**PR #3 でPhase 16C実装をmain反映済み**（merge commit `b96cc669ddc7c33f3563070d85962a07bb3b85fd`）、**PR #4 でExpo Go事前確認記録をmain反映済み**。versionCode 10 commit＝**`8706d7d3fcbfa349a1b71c4d45bb65e7e61fbb0a`**。**EAS preview APK versionCode 10＝FINISHED**（Build ID `10bedf3b-31c3-4fa7-aefe-73f62f960b5e`／APK `https://expo.dev/artifacts/eas/_fPMwwHwSgHXeHqTpmvN7iG5P6ZkeGKojNITU0WJ32o.apk`／artifact期限 2026-08-19T06:54:09Z）。**Pixelへ上書きインストール成功し、最終受入 K1〜K22 すべてPASS**（Metroなし単体起動・既存メモ/設定の保持・4系統からのOS共有起動・ChatGPT選択可・自動送信なし・キャンセル後復帰と文章維持・日本語/改行/長文保持・多重共有なし・誤った完了表示なし・守秘3チェック制御と本文変更後の全解除/再確認・**K22 実機でのコピー成功**）。**`33` Gate A〜G PASS／`32` Gate D PASS／Phase 16C最終完了**。ChatGPT未インストール端末の実機テストは実施していないが、**特定アプリを直接起動せずAndroid標準OS共有を利用する実装**であることを確認し、Gate基準を実装アーキテクチャに合わせて整理した（`33` §17.4・§23.7）。詳細は `33` §23.7・`32` §18.4・`10` §21・`docs/worklog/current.md`。再々レビューでは**H1解消・初回8件に回帰なし・`aria-checked` 問題なし・製品コード上の追加指摘なし**を確認済み（以降の文書レビュー経緯は `33` §23.3〜§23.4 と `docs/worklog/current.md` を参照）。**Phase 16Cは最終完了ではない**。**検証時系列（コミット単位）**：`78625c4`（文書整合）の回は任意の `npx tsc --noEmit` と `git diff --check` がいずれも合格し、古い表現の検索・章番号／内部参照の確認を実施。`c1828b5`（レビュー記録修正）の回は**tscを再実行せず**、`git diff --check`・文書検索・文書間の相互確認・見出し／内部参照の確認のみ実施。**今回（2026-08-05）の修正も同様にtscを再実行していない**（`git diff --check`・検索・相互確認・見出し／内部参照の確認のみ）。`expo export`・`expo-doctor`・Webブラウザ確認・単体43/43・H1のWeb確認48/48・回帰41/41・初回実装回帰83中81は、**いずれの回も再実行せず既存の検証結果を維持**（コード変更がないため）。記録：`33` §23・`10` §21・`docs/worklog/current.md`
* **Phase 16C 再レビュー対応（2026-08-02、Codex再レビューのHigh 1・Low 1を修正）**：対象HEAD `c541ab12bf702c3c6c795a69b44124090f9383f7` に対するCodex再レビュー＝Critical 0／**High 1**／Medium 0／**Low 1**／Note 4・判定「要修正・再レビュー」。**2件とも修正**（既存2コミットはamendせず追加コミット）。**H1（守秘チェックが編集後も残る）**＝`app/share/confirm.tsx` の本文変更処理で、**本文が実際に変わった場合のみ**（同じ文字列の再通知では何もしない）現場適応系（`workplace_question`／`workplace_report`）に限り守秘3チェックを全解除。解除後はコピー・共有が無効へ戻り、既存の必須確認文を再表示。3項目を再確認すれば再び有効。通常系（prompt／memo）は対象外、**保存は守秘チェックと連動させない**（`33` §20.5の範囲を拡大しない）。あわせて `accessibilityState.checked` がreact-native-webで `aria-checked` に反映されないことを確認し、チェックボックスへ `aria-checked` を明示追加。仕様は `33` §8.4.1 として安全補強を明文化（新機能・新Phaseではない）。**L1（Gate・現在状態の文書不一致）**＝`10` 16C-3のAndroid欄から Gate E（Web・確認済み）を除外しGate Dのみ未確認へ／`00_START_HERE.md`・`CLAUDE.md`・`AGENTS.md`・`docs/flowdock_mvp_design.md` §4.4.1・`01` §1.8・`35` §1.2・`08`/`14`/`16`/`21`/`29` のPhase 16見出しを現在状態（16A・16B実装済み／16Cコード実装済み・レビューとAndroid最終確認待ち）へ更新。過去の作業履歴・レビュー記録は歴史記録として変更しない。**検証**：`tsc --noEmit` 合格／`expo export --platform web` 成功／`expo-doctor` 17/18（既存パッチ差のみ）／`git diff --check` 合格／単体 **43/43**／H1パターンA〜F＋回帰のWeb確認 **48/48**／初回レビュー修正分の回帰 **41/41**／初回実装分の回帰 **83項目中81**（不合格2件はヘッドレスの `clipboard-write` 拒否＝既知の環境制約）。**DB・schema・migration・依存・lockfile・app.json・eas.json・versionCodeは無変更**。**Codex再々レビュー待ち／Expo Go事前確認・EAS APK更新・更新APKでのPixel最終確認・`33` Gate D・`32` Gate Dは未実施。Phase 16Cは最終完了としない**。記録：`33` §8.4.1・§22・`10` §21・`09` Phase 16・本ファイル・`docs/worklog/current.md`
* **Phase 16C レビュー対応（2026-08-02、Codex初回レビューのMedium 4・Low 4を修正）**：初回実装コミット `5e76a291a0eebad57b5e0285102ab3cedad7598b` に対するCodex初回レビュー＝Critical 0／High 0／**Medium 4**／**Low 4**／Note 4・判定「要修正・再レビュー」。**8件すべて修正**（初回コミットはamendせず追加コミット）。**M1 用途変更で編集内容を失う**＝生成直後の文章をbaselineとして保持し編集済みを判定。同一用途の再選択はno-op、未編集は即時変更、編集済みは既存 `confirmDialog` で確認（キャンセル時は用途・文章・結果表示を変更しない）。**M2 通常系保存の二重push**／**M3 4入口の遷移二重操作**＝共通hook `src/hooks/useNavigationLock.ts`（同期refロック・`useFocusEffect` で復帰時解除・同期例外時に解除＋後始末）を5経路へ適用。保存は「ロック→`setNoteDraftHandoff`→`router.push`」の順で、同期失敗時は `clearNoteDraftHandoff()`。**M4 保存後の状態**＝保存中は入力欄 `editable=false`・保存開始時点の文章を保存、文章変更で保存タイマー解除＋idle復帰＋再保存可。**L5 コピー結果**＝`useCopyFeedback` に `reset()` と世代管理を追加（進行中の結果は世代不一致なら表示せずidle・二重コピー防止は維持・後方互換）。**L6 ID取得失敗**＝`shareTargets.ts` を結果型化し「依頼文なし（用途5）」と「取得失敗」を型で区別。失敗時は本文だけへフォールバックせず理由表示＋コピー/共有/保存を無効（内部ID・本文は出さない）。**L7 `git diff --check`**＝`33` 冒頭の末尾2スペースを解消し、文書の記述を実際の結果へ訂正。**L8** `workplaceTags.ts` のコメントを「実装済み」へ。**検証**：`tsc --noEmit` 合格／`expo export --platform web` 成功／`expo-doctor` 17/18（既存パッチ差のみ）／`git diff --check` 合格（作業ツリー・`d1a0600..HEAD` とも exit 0）／単体 **43/43合格**（ID取得失敗の結果型を含む）／レビュー修正分のWeb確認 **41/41合格**／初回分のWeb回帰 **83項目中81合格**（不合格2件はヘッドレスの `clipboard-write` 拒否＝既知の環境制約）。**DB・schema・migration・依存・lockfile・app.json・eas.json・versionCodeは無変更**。**（この時点では）Codex再レビュー待ち／EAS APK更新待ち／Expo Go事前確認待ち／更新APKでのPixel最終確認待ち。現在状態は先頭項目を参照。Phase 16Cは最終完了としない**。記録：`33` §21・`10` §21・`09` Phase 16・本ファイル・`docs/worklog/current.md`
* **Phase 16C：外部AI・共有受け渡しの実装（2026-08-02、実装＋文書・初回実装コミット `5e76a29`）**：`33-external-handoff-and-sharing.md` を正本に、16C-1（共通共有基盤）／16C-2（プロンプト集・さくっとメモ）／16C-3（現場適応）を1バッチで実装。**新規**：`app/share/confirm.tsx`（route `/share/confirm`・出所表示／5用途／全文編集／通常注意または守秘3チェック／コピー／ChatGPTなどへ共有／新しい記録として保存／キャンセル）・`src/features/share/shareHandoff.ts`（共有対象と記録作成prefillを別スロットで保持。URL・DB・ログへ出さない。get＝純粋読み取り＋初回commit後clearでStrict Mode安全）・`src/features/share/shareTargets.ts`（5用途の固定マッピング・共有文章の組み立て・末尾入力待ち行の除去）・`src/utils/share.ts`（native＝RN標準`Share`／web＝`navigator.share`。新規依存なし）。**変更**：`app/_layout.tsx`（ルート登録・戻るフォールバック）・`app/prompts/index.tsx`・`app/memo/[id]/index.tsx`・`app/workplace/question.tsx`・`app/workplace/report.tsx`・`src/features/workplace/workplaceService.ts`（`saveReportNote` 追加。16B-3の `saveWorkplaceNote` を共用）・`app/notes/create.tsx`（prefill受け取り）・`src/components/WorkplaceSceneForm.tsx`（onSaveなし時も `saveHint` を上書き可能に。既定文言は不変）。**用途マッピング**：1=`thought`／2=`brain_dump_to_action`／4=`claude_prompt`（いずれも既存定義をIDで参照・複製なし）／3=**共有専用の固定依頼文**（既存42件に意味の合う定義がないための例外。`shareTargets.ts` の `QUESTION_ORGANIZE_REQUEST`。プロンプト集・`MOBILE_PROMPTS`・`NOTE_CATEGORIES`・Prompt Hub検索分類・DBテンプレート・`35` の編集対象に含めない）／5=本文のみ。**判断B**：守秘3チェックは共有確認画面内のコピー・共有にのみ必須。`/workplace/question`・`/workplace/report` の既存コピーは無変更。**DB・schema・migration・新規npm依存・app.json・eas.json・versionCodeは無変更**。**検証**：`tsc --noEmit` 合格／`expo export --platform web` 成功（`dist` が別プロセス使用中のため出力先をリポジトリ外へ指定）／`expo-doctor` 17/18（失敗1件は既存の `expo 54.0.35` vs `~54.0.36` パッチ差＝無関係）／`git diff --check` は**この時点で末尾空白の通知1件**（`33` 冒頭のMarkdownハード改行。レビュー修正コミットで解消）／用途マッピング単体 **32/32合格**／Web確認（ヘッドレスChrome・専用ポート8117・puppeteerはscratchpad隔離）**82項目中80合格**・console.error/pageerror/unhandledrejection 0。**未確認＝コピー成功経路（ヘッドレスの`clipboard-write`拒否＝環境制約。失敗表示と復帰は確認済み）・Android Expo Go・EAS APK更新・更新APKでのPixel最終確認・`33` Gate D／`32` Gate D**。**EAS APK更新は不要ではなく、Codexレビュー合格後にPhase 16全体を含む更新が必要**。Expo Go確認は事前確認で、最終確認は更新APKで行う。記録：`33` §20・`10` §21・`09` Phase 16・`11` §17.1・`docs/worklog/current.md`。作業ブランチ `feat/phase-16c-external-handoff-sharing`（main `d1a0600` から作成）。**PR作成・mainへのmergeは行わない**
* **さくっとメモのブレインダンプ整理・外部AI受け渡し拡張構想（35）の体系整備（2026-08-02、文書のみ・Codex再々レビューPASS・PR #2 merge済み・main反映済み）**：ユーザーが配置した新規構想文書 `docs/memo-app/35-quick-memo-brain-dump-and-ai-handoff.md`（構想の追加日は文書本文どおり2026-07-30）を、**未承認・未実装の構想**として文書体系へ整備済み。35は音声ブレインダンプ・整理待ち・本人整理欄・用途別プロンプト編集・課金境界・AI結果保存にまたがる横断構想で、Phase 16Cを置き換えない・33へ統合しない・**新Phase番号は作らない**。作業内容：35冒頭に未承認/未実装/16C後/承認済みは31・32・33の状態表示を強調、§1.1に関連正本（入力=`31`／共有=`33`／テンプレDB=`12`＝Phase 8〜9／プロンプト集=`14`／バックアップ=`34`）と責務分担・二重実装しない・共有ボタン名は`33`の「ChatGPTなどへ共有」を追記、4用途/5用途(`33`)/6入口(`14`)の未統合を明示、§18暫定結論を未承認と明示。参照追加：`00_START_HERE.md`・`CLAUDE.md`・`AGENTS.md`（読み分け表）・`09`(Phase 13将来候補)・`10`(§15.2)・`11`(§19 主な未決定)・本ファイル・`docs/worklog/current.md`。**コード・DB・依存・app.json・eas.json は無変更。記録当時はPhase 16C未開始（その後Phase 16Cは実装済み。先頭項目を参照）。新Phase番号なし。DB/migration追加なし**。**Codex初回レビュー＝要修正（High1：`33`確定の共有確認画面を35 §15で未決定へ戻していた／Medium2：5用途の節参照誤り（`33`§8.2→§7.2 SHARE-MEMO-02）・35冒頭の行末空白／Low1：未追跡ローカル設定）→ High・Medium修正済み。Codex再レビュー＝要修正（Medium1：用途選択と共有確認画面の同一/分離を35 §15で未決定へ戻していた→`33` §7.2・§8.1により用途選択は共有確認画面内で確定・§15/§5.4を修正／Note1：`.claude`記録を実測へ）→ Medium・Note修正済み。**Codex再々レビュー＝PASS（Critical/High/Medium/Low/Note すべて0件）。承認対象コミット `c9273646bcaeb820a7e5d8bf2e49dc9a20b19f2a`（`docs: add quick memo AI handoff concept`・文書9ファイルのみ）。**PR #2 は merge済み・closed。main反映済み（Phase 16C着手時の main／origin/main ＝ `d1a06003c89c52caa8ce8651bf9e15ee29458e06`）。旧作業ブランチはローカル・リモートとも削除済み**（35番は未承認・未実装の将来構想・Phase 16C後に実装範囲を判断。コード・DB・依存は無変更・新Phase番号なし。`.claude/settings.local.json`は未追跡・コミット対象外）
* **名称整理：利用者向け名称の MindHub／さくっとメモ統一（2026-07-27実装／2026-07-28 Codex最終再レビュー合格／PR #1 merge済み・main反映済み）**：利用者向けの現在の表記を、アプリ全体＝MindHub、`/memo` 系機能＝さくっとメモへ統一。FlowDock は旧名称・内部由来・旧形式互換・歴史的記録に限定し、新しい利用者向け表示では使用しない。**単純な一括置換はせず各出現の責務を確認**。**名称境界の詳細な正本は `01` §1.10**（§1.6・§1.7 は歴史的記録）。**状態**：初回実装をコミット→Codex初回レビューで「要修正・再レビュー」（互換は問題なし／旧名称方針が現行仕様書に規範文として残る・設定画面の説明不足・検索件数表記が指摘）→文書整合と設定画面補足を修正し amend→Codex2回目レビューのMedium指摘（`30` §14 UX-04表の「app.json変更は時期未確定」）を修正し amend→**Codex最終再レビュー合格（Critical/High/Medium/Low すべて0件）。合格対象コミット `1f368673337834336b98f4285a054b8da7a6de45`**。その後 PR #1 merge済み・main反映済み。**コード変更**：app.json `name`＝`"FlowDock"`→`"MindHub"`（識別子 `slug`/`scheme`/`package`/`projectId`/`versionCode` は維持）／`app/settings.tsx` の Repository 欄に補足文追加（プレースホルダ `例: FlowDock_Notes` は既存privateリポジトリ名の例示のため維持・既定値と誤認させない・GitHub設定/保存値/処理は無変更）。ホーム・Web document title は Phase 15/16A で移行済み＝追加変更なし。**文書変更**：`01` §1.6/§1.7 を歴史記録として明示＋§1.10 優先を冒頭で示す・§1.10 が現在正本／`28`（§11・§6.1・UX-04・§14・§17#6・§19）を §1.10 基準へ・app.json 変更「時期未確定」を解消／`11` の旧名称未解決項目を「決定済み・§1.10」へ／`08`・`30` の現行「クイックメモ」→「さくっとメモ」／`10` §18.1・`docs/worklog/current.md`・本ファイル。`CLAUDE.md`・`AGENTS.md`・`00_START_HERE.md` は初回で §1.10 へ更新済み（今回追加変更なし）。**維持（名称だけを理由に変更しない。詳細は §1.10）**：DB名 `flowdock.db`・`source: FlowDock`・`FlowDock_Notes`・`slug`/`scheme`・`android.package`・`projectId`・`versionCode`・npm名・由来コメント・歴史文書。**検索件数（`git grep`、大小無視、node_modules/dist/.git 除外）**：base `68c5e130`＝34ファイル/151 matching lines/163 occurrences（Codex報告「151箇所」＝matching lines相当）、修正後（最終）＝34ファイル/186 matching lines/232 occurrences（増加は本作業の記録追記による・ファイル数不変・利用者向け現在表示の残存FlowDockは0）。**検証**：`git diff --check` 問題なし・`tsc --noEmit` 合格・`expo export --platform web` 成功（Webタイトル `MindHub`）・`expo-doctor` は既存パッチ差のみ・設定画面Web確認合格（プレースホルダ維持・補足文表示・既存導線・console/pageerror/unhandledrejection 0）。**DB・migration・ルート・テーブル名・依存・EAS設定は無変更。名称整理実装完了・Codex最終合格・**PR #1 merge済み・main反映済み**。EAS APKビルドは未実施で、Androidランチャー表示名はPhase 16全体を含む次回の更新APKで最終確認する**。作業ブランチ `chore/mindhub-naming-cleanup`（合格対象コミット `1f36867` ＋本レビュー結果反映コミットの2コミット）。既存インストール済みAPKのランチャー名が FlowDock のままなのは既存APKに埋め込まれた名称であり今回のソース変更失敗ではない
* **バックアップ・復元・外部取り込み仕様（34）の既存文書反映（2026-07-27、文書整備完了）**：ユーザーが配置済みの新規正本 `docs/memo-app/34-backup-restore-and-external-import.md`（将来候補・実装時期未定・現在Phase対象外）を基準に、関連仕様書・ロードマップ・管理文書へ**参照と最小限の要約**を反映した。決定した境界＝完全バックアップはJSON／Markdownは閲覧・共有・個別取り込み用／`memos`・`notes`の正式復元はJSON／復元は元ID維持・共有コピーは新ID／IDへユーザー・端末情報を埋め込まない／`updated_at`だけで自動上書きしない／スマホ左右比較なし・自動マージなし／自動同期・共同編集は別の将来機能。**18の重複ID処理を改定**（空環境は元ID維持で追加／既存環境は同ID同内容スキップ・同ID内容相違は競合・`updated_at`だけで自動上書きしない・明示選択なしに置換しない・結果表示に競合件数追加）。更新ファイル：`00_START_HERE.md`・`01`(§1.9)・`03`(§9)・`07`(§15)・`09`(Phase 13)・`10`(§15.1)・`11`(§18)・`15`(§1.11〜1.14)・`16`(§9)・`17`(§6.3)・`18`(§1.1・§5・§6)・`docs/flowdock_mvp_design.md`(§6.1.1)・`current-tasks.md`・`docs/worklog/current.md`。未確定事項は `11` §18へ集約。**実装未着手＝コード・DB・依存・app.json・eas.json・versionCode変更なし。Phase 16の完了条件に影響なし。34全文の転載なし。`docs/architecture.md`・`src/**`・`app/**`は無変更**。commit・push完了：`4bd0839c990727fd730a4ca9ad0300884e42b731`
* **Phase 16B（質問タイミング判断支援）実装（2026-07-25、未コミット・commit承認待ち）**：現場適応モードに「質問タイミング判断支援」を追加。**16B-1／16B-2／16B-3を関連1機能としてまとめて実装**。仕様正本＝`32`。**16B-1 判定ロジック**＝`src/features/workplace/questionTiming.ts`（新規・純粋TS。React/Expo非依存）。3段階判定（すぐ確認する＞早めに質問する＞もう少し調査する）・危険条件優先・完全停止10分/通常20分の目安・現場ルール優先フラグ（判定とは独立）・理由は定型文（最大3）・investigate時のみ次に行うこと（未実施確認から最大3）・急ぎ度候補。危険/影響は「未回答／該当なし／1件以上」を型で区別（空配列を自動で該当なし扱いしない）。**16B-2 判定画面と導線**＝`app/workplace/question-timing.tsx`（新規・`/workplace/question-timing`・選択式のみ・自由文なし・KeyboardAvoidingView追加なし・ScrollView＋下部Safe Area）／詰まり記録→判定→質問の導線／`判定せず質問文を作る`（入力不足でも可）／入力変更で古い判定結果クリア／該当なし排他／直アクセスでクラッシュしない。引き継ぎ＝`src/features/workplace/workplaceHandoff.ts`（新規・A詰まり→判定／B判定→質問を型と関数で分離・各領域 set/get(純粋読み取り)/clear の3操作＝**Strict Mode安全**：`useState`初期化でget・初回commit後 `useEffect` で clear・URL/DB/ログ非露出）。**16B-3 完成質問文の任意保存**＝`saveQuestionNote`（`workplaceService.ts`。module-private `saveWorkplaceNote` を共用しprivate・Git候補false・`workplace,workplace_question`・type=thought・source=manual・title=`質問メモ YYYY-MM-DD` を強制。呼び出し側から解除不可）。`question.tsx` は引き継ぎを `useState`初期化でget＋`useEffect`でclear＋`onSave`＋`saveLabel="記録として保存"`＋`saveHint`上書き＋`banner`。`WorkplaceSceneForm` に optional `outputAction`（出力後の全幅追加操作）と `saveHint` を追加（未指定時は既存挙動維持＝他4場面・終業前は無変更）。`stuck.tsx` は `outputAction`「質問タイミングを確認」を追加（コピー維持）。`_layout.tsx` に `workplace/question-timing` の Stack.Screen（fallback `/workplace/stuck`）。`workplaceTags.ts` は古いコメントのみ更新（定数値不変）。**16Cの共有・OS共有・守秘3チェック・報告文保存は未実装（境界確認のみ）**。**判定ロジック単体確認**＝リポジトリ外scratchpadで実モジュールをtscコンパイル→node実行、`32` §15.2 の全ケースを網羅し**28/28合格**（危険1/複数→urgent・完全停止10-20→soon・通常20+→soon・期限/仕様→soon・全確認済→soon・危険×時間→urgent・安全＋未確認→investigate・現場ルール優先フラグ・急ぎ度候補・未回答≠該当なし）。ハーネスは確認後削除（残存0）。**機械確認**：`tsc --noEmit` 合格／`expo export --platform web` 成功（バンドルエラー0）／`expo-doctor` 17/18（失敗1件は既存の `expo 54.0.35` vs `~54.0.36` パッチ差＝無関係・未修正）／`git diff --check` 問題なし（LF/CRLF通知のみ）。**Web確認（ヘッドレスChrome・専用ポート8115・puppeteerはscratchpad隔離・確認後停止）**：6幅（360/390/412/480/768/1024）横スクロールなし・6入力群到達・下部操作到達・二重ヘッダーなし／詰まり→整理→質問タイミング→判定→質問文を作る の引き継ぎ（確認したいこと→聞きたいこと・状況→背景・エラー→背景末尾・試したこと→試したこと・判定→急ぎ度候補／checked空・decision空・判定理由は質問へ入らない・banner表示・URLに本文なし）／入力変更で結果クリア／3結果（urgent/soon/investigate）再現・各から質問へ／investigateは次に行うこと最大3／スキップ（入力不足でも可・詰まり内容引き継ぎ・急ぎ度空・理由なし）／保存（記録として保存・二重押下で二重保存なし・保存後aria-disabled・編集で再保存可・保存ヒントに終業前文言なし・コピー併用可）／**保存レコードを/notes詳細で確認＝type思考メモ・tags `workplace,workplace_question`・visibility private・Git候補対象外**／回帰（現場適応5カード・質問タイミングは入口カードでない・他4場面に追加操作なし・報告/開始はコピーのみ維持・終業前の保存ヒント不変・終業前保存動作・翌朝再開メモ表示・戻るで詰まり入力維持・現場適応へ戻る）／選択状態は色だけでなく☑☐◉○グリフ＋role併用／console.error・pageerror・unhandledrejection・VirtualizedList警告いずれも0。**Android Expo Go：Pixel実機確認済み・合格（2026-07-25）**（新規ネイティブ依存なし＝EAS APK不要）。**Gate A（判定単体）＝合格。Gate B（画面フロー）・C（保存）・E（回帰）＝Web＋単体＋Android Expo Go 合格。Gate D（共有・守秘）＝16Cとの連携Gートのため16B単体では対象外**。診断コード不使用（アプリコードに console.log/measureInWindow等なし）。**変更ファイル**：新規`src/features/workplace/questionTiming.ts`・`src/features/workplace/workplaceHandoff.ts`・`app/workplace/question-timing.tsx`／変更`app/_layout.tsx`・`app/workplace/stuck.tsx`・`app/workplace/question.tsx`・`src/components/WorkplaceSceneForm.tsx`・`src/features/workplace/workplaceService.ts`・`src/features/workplace/workplaceTags.ts`。DB・依存・app.json・eas.json・versionCode・schema無変更。**Strict Mode安全化（2026-07-25・commit前）**：引き継ぎの `consume`（読み取り＋削除）を `get`（純粋）＋`clear` に分離し、`useState`初期化ではgetのみ・`useEffect`でclear。実モジュールをscratchpadで再検証（handoff 17件＋判定28件＝45/45合格）、対象Web回帰24件合格（詰まり→判定→戻る→入力変更→再判定で最新入力・質問再訪で古い値残留なし・直アクセス残留なし・console/pageerror/unhandledrejection 0）。tsc・expo export web・expo-doctor 17/18再実行合格。**UX改善追加（2026-07-25・commit前／全画面遷移テスト38件で高重大度問題なし・完了導線不足を確認しての対応）**：(1) `WorkplaceSceneForm.handleBuild` 冒頭で `Keyboard.dismiss()`＝**整理する時にキーボードを閉じる**（全5場面共通。入力値・生成内容・保存/コピー処理・待機は無変更）。(2) `WorkplaceSceneForm` に optional `completionActions{primary,secondary}` を追加し、**質問文作成画面（`question.tsx`）だけ**に「完了後の移動」セクション（主要＝**ホームへ戻る**＝`router.dismissTo('/')`／補助＝**現場適応へ戻る**＝`router.dismissTo('/workplace')`）を追加。`dismissTo` で中間画面（質問タイミング・詰まり）や重複入口を閉じて戻す（back/push/replaceは使わない）。outputがある間だけ表示（保存前後・失敗後も維持）・入力変更で非表示・コピー/保存とは別セクション・全幅縦配置・44相当。**保存後の自動遷移は行わない**（保存後もコピー・ホーム/現場適応へ戻る可）。他4場面（start/stuck/report/end）には完了導線を追加しない。`dismissTo` は expo-router 6.0.24 の `router.dismissTo` を型定義で確認して採用。**追加分Web確認（専用ポート8114・8115維持・puppeteer隔離・確認後8114停止）**：表示条件（入力中非表示→整理する後表示→入力変更で消える→再生成で再表示）／ホームへ戻る＝`/`（フロー/カード/直アクセス各）／現場適応へ戻る＝`/workplace`（各）／保存前後にホーム・現場適応へ戻れる・保存直後の自動移動なし・保存後コピー可／回帰（stuckの質問タイミングを確認維持・他4場面に完了導線なし・question のコピー/保存維持・終業前保存＋翌朝再開・question-timingの戻る維持）／console.error・pageerror・unhandledrejection 0。ホーム/現場適応へ戻った後の**Webブラウザ戻るで質問・判定・詰まり画面が復活しないこと**を、要件どおりホーム起点フロー（`/`→クリック遷移）で内容ベース確認（8件合格＝いずれも復活せず）。※Androidハードウェア戻るでの非復活は `dismissTo` のネイティブ意味に一致するが**Android実機確認項目**として残す。**結果生成後の自動スクロール追加（2026-07-25・commit前）**：(1) `WorkplaceSceneForm.handleBuild` で `整理する` 後に**出力欄の先頭へ自動スクロール**（全5場面）。(2) `question-timing.tsx` の `判定する` で、判定成功時は**判定結果ブロック先頭**（現場ルール優先注意があればその先頭、なければ判定結果見出し）へ、入力不足時は**不足項目案内**へ自動スクロール。方式＝ScrollViewのref＋結果/出力欄の `onLayout` で取得したcontent内Y座標＋**明示操作直後だけ有効になる待ちフラグ**（`pendingBuildScrollRef`／`pendingJudgeScrollRef`）。`scrollTo({y: max(y - 余白, 0), animated:true})`。**固定setTimeout・measureInWindow・screenY/絶対座標比較・長さ推測・レンダーごとscrollToは不使用**（初回表示はonLayout＋待ちフラグ。**再押下＝結果表示済みで入力不変だとonLayoutが再発火しないため、ref保持したcontent内Y座標へ `requestAnimationFrame` で1回スクロールし、unmount時 `cancelAnimationFrame` で解除**。measureInWindow・screenY・絶対座標は不使用）。入力変更でoutput/結果が消えるとき・コピー・保存・保存成功/失敗・完了後の移動・戻る復帰・直アクセス・通常再レンダーでは**自動スクロールしない**。スキップ（判定せず質問文を作る）は別画面遷移のため対象外。**追加分Web確認（専用ポート8114・8115維持・puppeteer隔離・確認後8114停止）＝47件合格**：5場面で整理する後 出力見出しが上部（top 76〜257）・横スクロールなし・コピー到達・入力維持／長文でも出力見出し＋本文先頭が上部（末尾に飛ばない）／判定3結果（urgent/soon/investigate）＋現場ルール優先で結果ブロック先頭が上部・操作到達／入力不足で不足案内へスクロール・結果非生成・非クラッシュ／入力変更で結果消去＋下方向へ自動スクロールしない（scrollTopは自然clampのみ）・再判定で1回再スクロール／回帰（スキップ・保存・保存後コピー・ホーム/現場適応へ戻る・詰まりの質問タイミング導線・終業前保存＋翌朝再開）／console.error・pageerror・unhandledrejection 0。**再押下スクロール修正（2026-07-25・commit前）**：結果表示済みで入力不変のまま `整理する`／`判定する` を再押下するとonLayout非発火で再スクロールしない問題を確認し、Y座標ref＋rAF方式で修正（上記方式）。Web再確認＝再押下8件（整理する×3・判定する・入力不足・遅延誤スクロールなし）＋§6回帰15件（初回整理/判定で移動・不足案内へ・入力変更/コピー/保存で移動しない・ホーム/現場適応へ戻る・スキップ・console/pageerror/unhandledrejection 0）すべて合格。**Android Expo Go：Pixel実機確認済み・合格（2026-07-25）**（整理する後/判定後/不足案内へ移動・初回と再押下いずれも移動・入力変更/コピー/保存で誤スクロールなし・長文・日本語・整理する時のキーボード閉鎖・質問作成後のホーム/現場適応へ戻る・保存後の自動遷移なし・保存後コピー・dismissTo後のAndroid戻るで途中画面が復活しない・画面下部操作への到達をすべて確認）。変更ファイル：`src/components/WorkplaceSceneForm.tsx`・`app/workplace/question-timing.tsx`。記録：`32` §21・`10` §21・`docs/worklog/current.md`。**Phase 16B（16B-1／16B-2／16B-3）は実装・Web確認・Android Expo Go確認まで完了。EAS APK・iOS・Pixel以外のAndroid機種・3ボタンナビ・Gboard以外のキーボードは引き続き未確認。本内容を1commit（`feat: add workplace question timing guidance`）でcommit・push（＝今回のcommit対象）**
* **Phase 16A-1／16A-2 完了（2026-07-24実装・確認／2026-07-25時点でcommit・push済み）**：さくっとメモ作成・編集画面のAndroidキーボード操作性を改善。`react-native-keyboard-controller@1.18.5`＋`react-native-reanimated`（lock 4.1.7）＋`react-native-worklets@0.5.1` を導入（Babelは `babel-preset-expo` が自動設定・`babel.config.js` 新規なし）、`app/_layout.tsx` に `KeyboardProvider preload={false}`、作成・編集画面を単一 `KeyboardAvoidingView`（`behavior="height"`・Android限定・`keyboardVerticalOffset = insets.top + insets.bottom + spacing.sm(8dp)`＝実機計測でフッター上下余白12dp/12dp）でラップ。自動フォーカス（Android＝画面遷移後 `InteractionManager.runAfterInteractions` ＋ `requestAnimationFrame`／iOS＝`onAccessoryReady`／Web＝`onAccessoryReady`。固定 `setTimeout` なし・二重focus防止・アンマウント後focus防止）。編集画面は初回だけカーソルを既存本文末尾へ（selection一度制御→`onSelectionChange`で非制御へ）。詳細画面（`app/memo/[id]/index.tsx`・`app/notes/[id]/index.tsx`）の ScrollView に下部Safe Area余白（`paddingBottom: spacing.lg + insets.bottom`）を追加。**QMEMO-01〜09すべて合格（Android Expo Go実機確認）**。tsc合格・expo export(web)成功・Web回帰合格・expo-doctor 17/18（失敗1件は既存の `expo 54.0.35` vs `~54.0.36` パッチ差＝無関係）・git diff --check問題なし。一時診断コードは削除済み（残存0件）。**EAS APK未確認・iOS実機未確認・Pixel以外のAndroid機種未確認・3ボタンナビ環境未確認・Gboard以外のキーボード未確認**。**次の候補は Phase 16B／16C**。変更ファイル：`app/_layout.tsx`・`app/memo/create.tsx`・`app/memo/[id]/edit.tsx`・`app/memo/[id]/index.tsx`・`app/notes/[id]/index.tsx`・`package.json`・`package-lock.json`。記録：`31` §16・§17・`10` §21・`docs/worklog/current.md`。**commit・push済み（16A-1／16A-2 commit `d0e834be324f1c3909da3b1545c5eba7a215c269`。HEAD = origin/main = `d0e834b`、ahead/behind 0/0、working tree clean）。EASビルド・versionCode変更は未実施（未確認事項の確認は上記のとおり別途）**
* **Phase 16A-3 ホーム領域分離 完了（2026-07-24実装・確認／2026-07-25時点でcommit・push済み）**：`app/index.tsx` のみ変更。主要4カード（`FeatureCardGrid`）＋「最近のさくっとメモ」見出しを `FlatList` の `ListHeaderComponent` から取り出し、`AppHeader` と一覧の間の**固定 `View`（`fixedHomeSection`）**へ通常配置。一覧は `FlatList` へ `style={{flex:1, maxWidth:720, alignSelf:center}}`、`contentContainerStyle` に `flexGrow:1` を付与し、固定領域下の残り高さで**一覧だけをスクロール**（QMEMO-10〜12）。3件↔全件切替（`すべて表示`／`3件に戻す`）・FAB・maxWidth 720中央寄せ・カード名/順序/遷移/説明/accessibility・設定ボタン・AppHeader文言は無変更。sticky header/absolute配置/新ルート/新規部品/新規依存は不使用。取得・保存処理・DB・route・app.json・versionCodeは無変更。**機械確認**：`npx tsc --noEmit` 合格／`npx expo export --platform web` 成功／`npx expo-doctor` 17/18（失敗1件は既存のexpoパッチ版差 `54.0.36`期待/`54.0.35`検出＝本変更と無関係・未修正）／`git diff --check` 問題なし。**Web確認（ヘッドレスChrome・専用ポート8114・使い捨てuser-data-dir・puppeteer-coreはscratchpad隔離・確認後8114停止）**：6幅（360/390/412/480/768/1024）で横スクロールなし・4カード表示、空状態でカード＋見出し＋空メッセージ共存・カード表示順維持、UIから5件作成→初期3件→`すべて表示`で5件→`3件に戻す`で3件、スクロールで**カード固定（top 92不変）・見出し固定（role=heading top 295不変）・一覧のみスクロール**、console.error/pageerror/unhandledrejection/VirtualizedList入れ子警告いずれも0件。**未確認**：読み込み中・取得失敗・再試行の各状態はライブ再現せず**コードレビューのみ**（loading/error描画経路は基準版と同一・本変更で未改変）／FABと最終項目の重なりは精密未測定（`paddingBottom:96`維持・構造上非重複）。**EAS APK未確認**（ネイティブ依存・Android設定・SDK・ビルド設定は無変更のためEASビルド不要。Android Expo Go確認はPhase 16A-1・16A-2と合流後にPhase 16A全体で実施済み）。**Phase 16A-1（入力挙動）・16A-2（キーボード・フッター）も実装・確認・commit・push済み。Phase 16A（16A-1／16A-2／16A-3）は完了**（EAS APK等の未確認事項は上記のとおり残す）。記録：`10` §21 16A-3・`31` §16・本ファイル・`docs/worklog/current.md`。**commit・push済み（16A-3 commit `130910c`。HEAD = origin/main = `d0e834b`、ahead/behind 0/0、working tree clean）**
* **Phase 16文書統合 再レビュー残3点の修正（2026-07-24、文書のみ・未コミット）**：再レビューで残った3点を文書修正した。(1) `09` Phase 16Bの完了条件を「16B単体はGate A〜C・E／Gate Dは16C-1・16C-3統合時に確認する連携Gate／Gate D未通過だけで16B-1〜16B-3を未完了にしない」に修正 (2) `32` §8の導線注記を「詰まり記録または質問画面」→「質問タイミング確認画面」に修正（承認済みの詰まり記録→質問タイミング確認→質問文作成の配置・現場適応入口からの独立機能なしは不変） (3) `11` §17を未決定事項の参照のみへ整理（保存後の再編集は§14を正本として全文重複を排除・QMEMO-01〜14の実装/検証状態は`31`§16と`10`§21へ集約し§17から削除・解消済みレビュー履歴は`docs/worklog/current.md`へ・確定仕様は`01`/`31`〜`33`正本の記述は維持）。**実装は未着手**。アプリコード・DB・設定・依存は無変更。31〜33の製品判断は変更していない。commit・push未実施
* **Phase 16文書統合レビュー反映（2026-07-23、文書のみ・未コミット）**：Phase 16文書統合差分のレビューで、守秘・保存条件の競合／正本二重化／16B・16Cのタスク重複／共通共有ルート追加可否の曖昧さ／Android対象範囲の矛盾／既存実装と未実装タスクの区別不足／質問タイミング仕様の不整合／参照追記漏れが見つかり、**文書だけ**を修正した。主な修正：`23`（§5.1でPhase 16の質問・報告新規保存はprivate・Git候補false強制＝保存画面で変更させない／§6.1.1で一般メモの既定と区別・保存後の再編集の扱いは未決定として`11`へ）／`11`（§14の未決定から質問・報告の保存可否とタグを除外・§17を短縮し確定一覧は`01`・`31`〜`33`参照へ・「実質的矛盾は未検出」記述を削除）／`32`（導線を詰まり記録からに限定・「5群」→「6群」・QT-RULE-01から現場ルール即時確認の自動判定を除外・Gate Dをコピー/共有不可＝連携Gateと明記・16B-3を保存のみに）／`33`・`09`・`10`（16B-3=完成質問文の任意保存のみ、16C-3=共有接続＋報告文保存＋守秘チェック・二重実装しない・依存/実装順明記）／`01`・`CLAUDE.md`・`29`・`10`（既存ルート保護と`33`§13共通共有ルート追加可を分離）／`20`（Android APK版はPhase 16の実装・検証対象、オフライン拡張・端末間同期は将来候補のまま）／`31`（§16 要件別実装状態表を追加＝QMEMO-08等は実装済み・回帰確認、QMEMO-10〜14未実装）／`10`（16A-2は既存保存処理を書き直さない）／`21`（参照表記修正）／`02`（§11 Phase 16参照節を追加）。**実装は未着手**。DB・コード・設定・依存・ルート実装は無変更。31〜33の製品判断は変更していない。commit・push未実施。**次の作業＝再レビュー**
* **Phase 16仕様書追加・既存仕様統合（2026-07-23、文書のみ・未コミット）**：ユーザーとChatGPTが作成・承認済みのPhase 16新規仕様書3件（`docs/memo-app/31-quick-memo-usability-improvement.md`＝16A／`32-workplace-question-timing-rules.md`＝16B／`33-external-handoff-and-sharing.md`＝16C）を、既存仕様体系へ参照・要約・Phase・タスク・守秘境界として統合した。**実装は未着手＝文書だけ**。更新した既存文書：`00_START_HERE.md`・`CLAUDE.md`・`docs/flowdock_mvp_design.md`・`docs/memo-app/01`（§1.8追加決定）・`03`（§8 スキーマ変更なし）・`04`（§1.2.1 質問=`workplace,workplace_question`／報告=`workplace,workplace_report`）・`06`（§1.2 OS共有導線）・`07`（§14 OS共有とGitの境界）・`08`（§11 横断導線）・`09`（Phase 16）・`10`（§21 タスク16A-1〜16C-3）・`11`（§17 確定事項）・`14`（§1.9）・`16`（§8 プラットフォーム差）・`17`（§6.2）・`20`〜`23`（質問タイミング・保存確定・守秘チェック）・`28`（§18.1 参照）・`29`（§10.1 共通ルール）・`30`（§19.1 境界注記）。新規仕様全文の転載はせず要約・参照のみ。DB・コード・ルート・依存・設定は無変更。EASビルド・APK作成なし。**次の作業＝文書差分レビュー（ユーザー）**。commit・pushは未実施
* **現場適応キーボード `on-drag` 副作用修正＝Expo Go実機合格（2026-07-15）**：versionCode 8 APKの実機で、`/workplace/stuck` の入力中に上へスクロールすると `keyboardDismissMode="on-drag"` によりキーボードが閉じ→追加下部余白も消えてスクロールできない副作用が判明。**`src/components/WorkplaceSceneForm.tsx` の1行のみ** `keyboardDismissMode` を `"on-drag"`→**`"none"`** に変更（キーボード高さ取得・0復帰・cleanup・`paddingBottom: 40 + keyboardHeight`・非表示時40・`keyboardShouldPersistTaps="handled"` は維持。KeyboardAvoidingView・softwareKeyboardLayoutMode・固定大余白・依存・自動フォーカス移動・画面別重複実装なし）。**ユーザーがPixelのExpo Go（LAN・専用ポート8115・他スレッド無停止）で合格確認**：キーボード表示したままスクロール可／スクロール開始で閉じない／下側入力欄と「整理する」まで到達／入力維持／キーボード非表示後の余白復帰。tsc合格・`expo export`（web）成功・`git diff --check`問題なし。発表デモに影響するキーボード問題は解決確認済み。**引き続き未確認・未完了＝TalkBack／最大フォント見切れ／コピー成功経路／GitHub実通信／Gate 6・Gate 7**。未追跡 `AGENTS.md` は作業対象外＝無変更。記録：`30` §8.10・`11` §16・`docs/worklog/current.md`・`current-tasks.md`。この後：修正commit→versionCode 9→push→EAS preview APK
