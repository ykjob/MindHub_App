...
* **Phase 15の次に行う1作業**：バッチ3＝メモ管理一覧改修（NOTES-01〜04）**前の調査・範囲確認**（既存 `/notes` 一覧の実装調査と改修対象の整理。バッチ3実装はその後・未着手）。保留事項（戻るボタン視認性・FlowDock作成画面の下部余白・FilterChipのnotes適用）は引き続き対象外
